...
export function inferStructuredProfiles(
  ...
  const changelog = inferChangelogProfile(input.path, sample);
  if (changelog) {
    profiles.push(changelog);
  }
  if (hasMadrEvidence(input.path, sample)) {
    profiles.push('madr');
  }
  return profiles;
}
...
function hasMadrEvidence(path: string, text: string): boolean {
  const normalizedPath = path.replace(/\\/g, '/');
  const hasPath = /(^|\/)(docs\/decisions|decisions)\//i.test(normalizedPath);
  const hasFilename = /\/\d{4}-[a-z0-9]+(?:-[a-z0-9]+)*\.md$/i.test(normalizedPath);
