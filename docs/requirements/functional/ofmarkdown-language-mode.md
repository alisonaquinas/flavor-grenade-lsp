...
6. Verify the selector writes `.mdfattributes` for the chosen scope and updates without changing the language id.
...
**Owner:** flavor-grenade-lsp contributors.
**Source:** [[docs/features/ofmarkdown-language-mode]], [[docs/features/markdown-flavor-config-files]], [[docs/adr/ADR021-mdfignore-mdfattributes-flavor-configuration]], [[docs/adr/ADR020-markdown-flavor-selection]].
...
**User Req:** User.Extension.SelectMarkdownFlavor
**Gist:** The selector, `.mdfattributes` flavor attribute, and server-facing flavor model must include every Markdown flavor researched in `docs/research/`, plus an `auto` mode.
...
**Scale:** Number of supported flavor ids present in the selector, `.mdfattributes` parser, and client/server initialization payload.
**Meter:**
...
4. Verify each id is accepted by `.mdfattributes` `flavor=<id>`.
...
3. Open a Markdown file matched by `.mdfattributes` with no explicit in-memory selector state.
...
7. Open a directory tree with no `.mdfignore` or `.mdfattributes` and verify every Markdown file in that tree uses Auto Detect.
8. Verify a matching `.mdfattributes` `flavor=auto` rule also runs Auto Detect for that path.
9. Compute: (correct auto resolutions / total contexts tested) x 100.
**Fail:** Auto detection selects Obsidian for generic Markdown without a vault/config signal, ignores a matching `.mdfattributes` rule, or fails to select Obsidian for `.obsidian/` vault notes.
**Goal:** 100% documented auto-detection correctness.
**Stakeholders:** Vault authors, Markdown authors.
**Owner:** flavor-grenade-lsp contributors.
**Source:** [[docs/design/markdown-flavor-auto-detection]], [[docs/requirements/functional/workspace]], [[docs/features/markdown-flavor-config-files]], [[docs/adr/ADR021-mdfignore-mdfattributes-flavor-configuration]], [[docs/adr/ADR020-markdown-flavor-selection]], [[docs/features/ofmarkdown-language-mode]].
...
**Gist:** User flavor overrides must persist to `.mdfattributes` at selected-file or active-directory scope.
...
**Scale:** Percentage of override operations written to the correct `.mdfattributes` pattern and attribute target.
**Meter:**

1. Open a Markdown file inside a workspace folder.
2. Select `CommonMark` in the flavor selector.
3. Choose `Selected file` and verify `.mdfattributes` in the active file's directory contains a file-specific `flavor=commonmark` rule.
4. Select `GitHub Flavored Markdown`.
5. Choose `All files in this directory` and verify `.mdfattributes` contains a directory-local Markdown pattern with `flavor=gfm`.
6. Open a standalone Markdown file with no workspace folder.
7. Select `Original Markdown`, choose `Selected file`, and verify `.mdfattributes` is written beside the file.
...
**Fail:** A selector override is written to VS Code flavor settings, the wrong `.mdfattributes` file, or a pattern broader than the selected scope.
**Goal:** 100% correct `.mdfattributes` persistence scope.
**Stakeholders:** VS Code users, teams sharing repository config, extension maintainers.
**Owner:** flavor-grenade-lsp contributors.
**Source:** [[docs/adr/ADR021-mdfignore-mdfattributes-flavor-configuration]], [[docs/features/markdown-flavor-config-files]], [[docs/adr/ADR020-markdown-flavor-selection]].
...
**User Req:** User.Extension.TrustFlavorBehavior
**Gist:** Files matched by `.mdfignore` must be inactive for Flavor Grenade and absent from server indexes.
...
1. Create `.mdfignore` fixtures for direct file patterns, directory patterns, `**` patterns, anchored patterns, and negated re-inclusion.
...
5. Modify `.mdfignore` to re-include a file and verify the file becomes visible after refresh.
...
**Owner:** flavor-grenade-lsp contributors.
**Source:** [[docs/features/markdown-flavor-config-files]], [[docs/adr/ADR021-mdfignore-mdfattributes-flavor-configuration]], [[docs/design/markdown-flavor-auto-detection]].
...
3. Write `.mdfattributes` rules for each required explicit flavor id.
4. Verify the server receives a file-change or refresh signal and re-analyzes open documents with that flavor id.
5. Override a single-file context to `original` through `.mdfattributes`.
...
**Stakeholders:** Markdown authors, extension maintainers, server maintainers.
**Owner:** flavor-grenade-lsp contributors.
**Source:** [[docs/design/markdown-flavor-auto-detection]], [[docs/design/api-layer]], [[docs/adr/ADR021-mdfignore-mdfattributes-flavor-configuration]], [[docs/adr/ADR020-markdown-flavor-selection]].
