...
Persistent file and directory flavor assignment is provided by `.mdfattributes`.
File visibility is provided by `.mdfignore`. Files ignored by `.mdfignore` are
...
**Gist:** The server must resolve `.mdfignore` and `.mdfattributes` before parsing or indexing a Markdown file.
...
1. Create fixture vaults with root and nested `.mdfignore` and `.mdfattributes` files.
...
4. Verify visible files receive the expected `EffectiveMarkdownContext` from `.mdfattributes`.
