...
| **MarkdownFlavorProfile** | Config / shared contract | Source-backed metadata for one explicit `MarkdownFlavorId`: label, display order, syntax capabilities, host behavior notes, and research trace. Parser and extension both consume this shared contract; BC2 does not own UI labels. |
