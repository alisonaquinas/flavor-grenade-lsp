...
|------|----------------|------------|
| **Vault** | BC4 Vault & Workspace | A Markdown workspace root detected by `.obsidian/` or a Markdown flavor config-file marker (`.mdfignore` or `.mdfattributes`). A vault is the natural unit of reference resolution: document links and dialect-specific references are resolved within a single vault unless a dialect profile states otherwise. |
...
| **MarkdownFlavorSelection** | BC4 Vault & Workspace / Config | The configured selector value resolved from `.mdfattributes`. Values are `auto` plus each supported explicit `MarkdownFlavorId`. It is input to effective flavor resolution, not UI-only state. |
...
| **VaultDetector** | BC4 Vault & Workspace | The domain service that decides whether a given directory path constitutes a vault. Detection criteria: presence of `.obsidian/` subdirectory OR a Markdown flavor config-file marker (`.mdfignore` / `.mdfattributes`). |
...
| **MarkdownFlavorCascade** | Config / BC4 Vault & Workspace | The named resolution order for the base `EffectiveMarkdownFlavor`: `.mdfignore` visibility gate, `.mdfattributes`, Auto Detect signals, CommonMark fallback. Structured profile resolution runs beside this cascade and writes profile flags into `EffectiveMarkdownContext`. |
...
| **SelectorState** | BC6 Editor Client | The client-side UI/config state shown to the user: `auto` or an explicit `MarkdownFlavorId`, plus separate structured-profile selection when configured. It is persisted through `.mdfattributes` and refreshed through the server. It is not the authoritative `EffectiveMarkdownContext`. |
...
| **MarkdownFlavorController** | BC6 Editor Client | The extension component that observes open/visible Markdown documents and writes `MarkdownFlavorSelection` changes through scoped `.mdfattributes` rules. It preserves VS Code's `markdown` language id and does not own server effective flavor state. |
