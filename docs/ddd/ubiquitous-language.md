...
| **MarkdownFlavorCascade** | Config / BC4 Vault & Workspace | The named resolution order for the base `EffectiveMarkdownFlavor`: VS Code explicit override, VS Code workspace-folder/workspace setting, project `.flavor-grenade.toml`, vault marker, CommonMark fallback. Structured profile resolution runs beside this cascade and writes profile flags into `EffectiveMarkdownContext`. |
