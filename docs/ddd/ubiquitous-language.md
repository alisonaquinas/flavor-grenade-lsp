...
|------|----------------|------------|
| **Vault** | BC4 Vault & Workspace | A Markdown workspace root detected by `.obsidian/` or a Flavor Grenade config-file marker (`.fgignore` or `.fgattributes`). A vault is the natural unit of reference resolution: document links and dialect-specific references are resolved within a single vault unless a dialect profile states otherwise. |
...
| Term | Bounded Context | Definition |
|------|----------------|------------|
| **MarkdownDoc** | BC2 Document Lifecycle | The aggregate representing a single Markdown document under an explicit `EffectiveMarkdownContext`. Encapsulates raw text, parsed structure (CST + AST), derived `MarkdownIndex`, and version tracking. Identity: `DocId`. Current code may expose this as `OFMDoc` while Obsidian is the only behavior-bearing projection. |
...
| **ParseContext** | BC2 Document Lifecycle | Immutable parse input metadata: `DocId`, version source, base `EffectiveMarkdownFlavor`, matching `MarkdownFlavorProfile`, and structured profile flags. BC4 supplies it; BC2 consumes it. BC2 does not choose the effective flavor or structured profiles. |
...
| Term | Bounded Context | Definition |
|------|----------------|------------|
| **VaultFolder** | BC4 Vault & Workspace | The aggregate representing a single detected vault. Identity: `VaultRoot`. Owns the document collection (`Map<DocId, MarkdownDoc>`; current code may name it `OFMDoc`), the `RefGraph`, the merged `FlavorConfig`, effective Markdown context state, and the `FolderLookup` index. |
...
| **MarkdownFlavorSelection** | BC4 Vault & Workspace / Config | The configured selector value resolved from `.fgattributes`. Values are `auto` plus each supported explicit `MarkdownFlavorId`. It is input to effective flavor resolution, not UI-only state. |
| **EffectiveMarkdownFlavor** | BC4 Vault & Workspace | The explicit base `MarkdownFlavorId` selected for a document after applying `MarkdownFlavorCascade`. It is never `auto` and is one part of `EffectiveMarkdownContext`. |
| **StructuredMarkdownProfileId** | Config / BC4 Vault & Workspace | An independent structured-document profile flag such as `keep-a-changelog`, `common-changelog`, or `madr`. It layers over the base Markdown flavor and is not a `MarkdownFlavorId`. |
| **EffectiveMarkdownContext** | BC4 Vault & Workspace | The document-specific parse context BC4 owns and supplies to BC2: one explicit `EffectiveMarkdownFlavor`, the matching `MarkdownFlavorProfile`, and zero or more compatible structured profile flags. |
...
| **VaultDetector** | BC4 Vault & Workspace | The domain service that decides whether a given directory path constitutes a vault. Detection criteria: presence of `.obsidian/` subdirectory OR a Flavor Grenade config-file marker (`.fgignore` / `.fgattributes`). |
...
| **SingleFileMode** | BC4 Vault & Workspace | The operating mode when neither `.obsidian/` nor a Flavor Grenade config-file marker is found. A single document is tracked without vault context. Intra-document refs resolve; cross-document refs are all unresolved. Evicted when a vault enclosing the file is subsequently detected. |
...
| **ConfigCascade** | Config | The ordered search path for operational configuration: (1) project config at vault root, (2) user config, (3) built-in defaults. Lower entries are overridden by higher entries. File/directory flavor assignment is not part of this cascade. |
...
| **MarkdownFlavorCascade** | Config / BC4 Vault & Workspace | The named resolution order for the base `EffectiveMarkdownFlavor`: `.fgignore` visibility gate, `.fgattributes`, Auto Detect signals, CommonMark fallback. Structured profile resolution runs beside this cascade and writes profile flags into `EffectiveMarkdownContext`. |
...
| **SelectorState** | BC6 Editor Client | The client-side UI/config state shown to the user: `auto` or an explicit `MarkdownFlavorId`, plus separate structured-profile selection when configured. It is persisted through `.fgattributes` and refreshed through the server. It is not the authoritative `EffectiveMarkdownContext`. |
...
| Term | Bounded Context | Definition |
|------|----------------|------------|
| **ExtensionClient** | BC6 Editor Client | The VS Code extension that wraps the LSP server. Responsible for server resolution, LanguageClient lifecycle, status bar, and Command Palette integration. Thin client; all language intelligence lives in the server. |
| **ServerResolver** | BC6 Editor Client | The 2-tier resolution strategy for starting the server: (1) user or machine setting `flavorGrenade.server.path`, with workspace values ignored, (2) bundled JavaScript server module at `server/main.js` relative to extension root. No PATH fallback, no download. |
...
| **MarketplaceVSIX** | BC6 Editor Client | The `.vsix` package containing the extension client JS bundle and one bundled JavaScript server module. Native server executables are excluded from Marketplace payloads. |
| **MarkdownFlavorController** | BC6 Editor Client | The extension component that observes open/visible Markdown documents and writes `MarkdownFlavorSelection` changes through scoped `.fgattributes` rules. It preserves VS Code's `markdown` language id and does not own server effective flavor state. |
