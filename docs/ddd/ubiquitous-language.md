...
|------|----------------|------------|
| **Vault** | BC4 Vault & Workspace | A Markdown workspace root detected by `.obsidian/` or a Flavor Grenade project config marker. A vault is the natural unit of reference resolution: document links and dialect-specific references are resolved within a single vault unless a dialect profile states otherwise. |
...
| **MarkdownFlavorSelection** | BC4 Vault & Workspace / Config | The configured selector value received from VS Code or project config. Values are `auto` plus each supported explicit `MarkdownFlavorId`. It is input to effective flavor resolution, not UI-only state. |
...
| **VaultDetector** | BC4 Vault & Workspace | The domain service that decides whether a given directory path constitutes a vault. Detection criteria: presence of `.obsidian/` subdirectory OR a Flavor Grenade project config marker. |
...
| **ConfigCascade** | Config | The ordered search path for configuration: (1) project config at vault root, (2) user config, (3) built-in defaults. Lower entries are overridden by higher entries. |
...
| **MarkdownFlavorCascade** | Config / BC4 Vault & Workspace | The named resolution order for the base `EffectiveMarkdownFlavor`: VS Code explicit override, VS Code workspace-folder/workspace setting, project config, vault marker, CommonMark fallback. Structured profile resolution runs beside this cascade and writes profile flags into `EffectiveMarkdownContext`. |
...
| **SelectorState** | BC6 Editor Client | The client-side UI/config state shown to the user: `auto` or an explicit `MarkdownFlavorId`, plus separate structured-profile selection when configured. It is persisted through VS Code configuration and sent to the server. It is not the authoritative `EffectiveMarkdownContext`. |
