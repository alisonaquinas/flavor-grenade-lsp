...
| **MarkdownFlavorSelection** | BC6 Editor Client | The selected Markdown dialect for a `markdown` document. Values include `auto` plus every supported flavor id derived from the research corpus. |
| **MarkdownFlavorController** | BC6 Editor Client | The extension component that observes open/visible Markdown documents, evaluates vault/index membership signals, applies user or workspace overrides, and propagates effective flavor state while preserving VS Code's `markdown` language id. |
