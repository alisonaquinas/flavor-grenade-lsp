...
| `VaultFolder.withMarkdownFlavorSelection` | `(folder: VaultFolder, selection: MarkdownFlavorSelection, scope?: DocId) → VaultFolder` | Store a validated selector from `.mdfattributes` scope, recompute affected `EffectiveMarkdownContext` values, and mark changed docs for reparse. |
...
| I6 | Workspace owns resolved `.mdfignore` / `.mdfattributes` resource state plus `EffectiveMarkdownContext`; BC5 validates and dispatches refresh inputs but does not store them. |
...
| `Workspace.withMarkdownFlavorSelection` | `(ws: Workspace, selection: MarkdownFlavorSelection, scope?: VaultRoot \| string) → Workspace` | Store validated `.mdfattributes` flavor state, recompute effective context for affected vault or single-file docs, and schedule reparse/diagnostic refresh for changes. |
| `Workspace.withStructuredProfileSelection` | `(ws: Workspace, selection: StructuredProfileSelection, scope?: VaultRoot \| string) → Workspace` | Store validated `.mdfattributes` structured-profile state, recompute effective context for affected vault or single-file docs, and schedule reparse/diagnostic refresh for changes. |
...
`MarkdownFlavorCascade` is executed by BC4 whenever a vault is detected,
`.mdfignore` or `.mdfattributes` changes, or BC5 dispatches a validated
resource-specific refresh input. It returns the base `EffectiveMarkdownFlavor`
for an `EffectiveMarkdownContext`. The normative precedence and
resource-specific flow are defined in
[[docs/design/markdown-flavor-auto-detection]].

```text
1. `.mdfignore` visibility gate
2. `.mdfattributes` `flavor` attribute
...
- Ignored files are inactive and never reach parser/indexing services.
- Deeper `.mdfattributes` files and later matching rules override broader earlier
  rules per attribute.
- `auto` and `!flavor` delegate to Auto Detect.
...
```text
1. `.mdfattributes structured_profiles`
2. Strong local structured-profile evidence
...
VaultDetectionResult
  ...
  | { kind: 'flavor-config'; root: VaultRoot } — .mdfignore or .mdfattributes marker found
...
2. Check for .mdfignore or .mdfattributes marker files in {dir}
...
> [!NOTE]
> The `vault_detection` config key controls which detection signals are respected. Default is `"obsidian"` — only `.obsidian/` triggers vault mode. Set to `"both"` in non-Obsidian editors that use `.mdfignore` or `.mdfattributes` marker files.
