...
This document is the authoritative domain model for **Bounded Context 6: Editor Client**. BC6 is a Generic Support subdomain. It contains no language intelligence or domain logic of its own — it is a thin wrapper (~200 lines) that resolves the server binary, manages the `LanguageClient` lifecycle, and wires up a status bar widget and Command Palette commands. All Obsidian Flavored Markdown intelligence lives in the server (BC2–BC5).
...
5. Push all disposables to `context.subscriptions` for automatic cleanup.
...
activate(context: ExtensionContext)
  ...
  │    clientOptions: { documentSelector: [{ scheme: 'file', language: 'markdown' }] }
  │
  ├─ client.start()
