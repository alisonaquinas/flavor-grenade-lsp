...
|---|---|
| Run date | 2026-05-13 |
| Runner / command | Codex local Phase 28 gate execution |
| Commit under test | `49d2a5c` |
...
| `bun test src/parser/__tests__/markdown-flavor-parser-analysis.test.ts` | Pass | Parser analysis tests passed, including Phase 28 MultiMarkdown syntax and implemented surface status. |
| `bun test src/test/integration/markdown-flavor.test.ts` | Pass | Spawned-server flavor tests passed, including Original/CommonMark/Obsidian/GFM/GLFM/Pandoc behavior and Phase 28 MultiMarkdown syntax counts. |
...
| `bun test src/` | Pass | 740 tests passed. |
| `bun test src/test/integration/` | Pass | 24 integration tests passed. |
...
| MF-VF-007 | Pass | Host-boundary review artifact exists and records deferred non-local behavior rules plus Phase 22 Original Markdown, Phase 23 CommonMark, Phase 24 Obsidian, Phase 25 GFM, Phase 26 GLFM, Phase 27 Pandoc, and Phase 28 MultiMarkdown dispositions. |

## Notes

This run is root/server Phase 28 evidence. It does not
