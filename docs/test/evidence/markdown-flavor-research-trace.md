...
| Sanitization | Repository-relative paths and source slugs only; no note bodies, `.mdfignore`/`.mdfattributes` contents, environment variables, tokens, local user paths, or raw server output included |
...
Phase 23 replaces the `commonmark` planned surface entries with implemented
local CommonMark behavior. Evidence is recorded in
`src/parser/__tests__/markdown-flavor-parser-analysis.test.ts`,
`src/resolution/__tests__/diagnostic-service.test.ts`,
`src/completion/__tests__/completion-router.test.ts`,
`src/test/integration/markdown-flavor.test.ts`, and
...
Phase 24 replaces the `obsidian` planned surface entries with implemented
local Obsidian behavior. Evidence is recorded in
`src/parser/__tests__/markdown-flavor-parser-analysis.test.ts`,
`src/resolution/__tests__/diagnostic-service.test.ts`,
`src/completion/__tests__/completion-router.test.ts`,
`src/test/integration/markdown-flavor.test.ts`, and
...
Phase 25 replaces the `gfm` planned surface entries with implemented local
GFM behavior for the published syntax subset. Evidence is recorded in
...
Phase 27 replaces the `pandoc` planned surface entries with implemented local
Pandoc behavior for source-backed syntax that does not require conversion.
Evidence is recorded in `src/parser/pandoc-parser.ts`,
`src/parser/__tests__/markdown-flavor-parser-analysis.test.ts`,
`src/resolution/__tests__/diagnostic-service.test.ts`,
`src/completion/__tests__/completion-router.test.ts`,
`src/handlers/__tests__/document-symbol.handler.test.ts`,
...
Phase 28 replaces the `multimarkdown` planned surface entries with implemented
local MultiMarkdown behavior for source-backed syntax that does not require
conversion or generated output. Evidence is recorded in
...
Phase 29 replaces the `mdx` planned surface entries with implemented local MDX
behavior for source-backed syntax that does not require compilation, bundlers,
dynamic imports, or React/TypeScript symbol lookup. Evidence is recorded in
...
Phase 31 replaces the `markdown-extra` planned surface entries with
implemented local Markdown Extra behavior for source-backed syntax that does
not require PHP Markdown Extra execution, HTML conversion, renderer output,
generated metadata, or syntax highlighters. Evidence is recorded in
...
Phase 33 replaces the `reddit` planned surface entries with implemented local
Reddit Markdown behavior for source-backed syntax that does not require Reddit
API calls, live user/subreddit/post/comment lookup, moderation state, or Rich
Text editor rendering. Evidence is recorded in
