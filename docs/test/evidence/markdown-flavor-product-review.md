...
| Sanitization | Repository-relative paths only; no note bodies, `.mdfignore`/`.mdfattributes` contents, environment variables, tokens, local user paths, or raw server output included |
