...
|---|---|---|
| MF-E-001 | `docs/bdd/features/ofmarkdown-language-mode.feature` | `.md` files stay `markdown`; selector exposes every required flavor; workspace-folder/workspace/user overrides persist; auto-detection clears and recomputes from TOML, marker, syntax/context inference, ambiguity fallback, and CommonMark fallback; `workspace/didChangeConfiguration` propagation is recorded with configured and effective flavor. |
