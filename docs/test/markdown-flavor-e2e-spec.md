...
|---|---|---|
| MF-E-001 | `docs/bdd/features/ofmarkdown-language-mode.feature` | `.md` files stay `markdown`; selector exposes every required flavor; selected-file and directory `.fgattributes` writes persist; Auto Detect clears/resets the chosen scope and recomputes from marker, syntax/context inference, ambiguity fallback, and CommonMark fallback; server refresh is recorded with selected and effective flavor. |
