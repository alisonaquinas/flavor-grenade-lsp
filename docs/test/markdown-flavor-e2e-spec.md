...
`.mdfattributes` writes, `.mdfignore` inactive state, user-visible selector state,
...
|---|---|---|
| MF-E-001 | `docs/bdd/features/ofmarkdown-language-mode.feature` | `.md` files stay `markdown`; selector exposes every required flavor; selected-file and directory `.mdfattributes` writes persist; Auto Detect clears/resets the chosen scope and recomputes from Obsidian marker, syntax inference, ambiguity fallback, and CommonMark fallback; server refresh is recorded with selected and effective flavor. |
...
| MF-E-006 | planned structured-profile feature scenarios | Keep a Changelog, Common Changelog, and MADR examples under `.mdfattributes`-configured and config-absent inference smoke workspaces resolve as structured profile flags layered over the active base flavor, without adding selector choices or enabling both changelog variants for one document. |
...
- Auto-detection scenarios include syntax inference and boundary
  confinement, and prove Auto Detect is independent from `.mdfattributes`
...
- E17 remains the owner of real VS Code host proof for document selector,
  activation, `.mdfattributes` persistence, `.mdfignore` inactive state,
