...
| `src/handlers/__tests__/document-symbol.handler.test.ts` | Unit | Tests document symbols for headings, block anchors, GFM tables/tasks, GLFM description lists/TOC tags, Pandoc metadata/labels/footnotes, MultiMarkdown metadata/labels/citations/footnotes, MDX ESM/JSX/expression symbols, kramdown attribute/definition-list/table/footnote symbols, and Markdown Extra attribute/definition-list/table/footnote/abbreviation symbols | `Parity.StructuralLSP.Coverage`, `FlavorLSP.Navigation.ProfileResolution` | Phase 17, Phase 25, Phase 26, Phase 27, Phase 28, Phase 29, Phase 30, Phase 31 |
| `src/handlers/__tests__/folding-range.handler.test.ts` | Unit | Tests structural folding ranges for frontmatter, headings, callouts, opaque code, math, comments, Templater regions, GFM table blocks, GLFM description lists, Pandoc fenced Divs/definition lists, MultiMarkdown metadata/table blocks, MDX JSX/expression blocks, kramdown definition-list/table/math blocks, and Markdown Extra definition-list/table/fenced-code blocks | `Parity.StructuralLSP.FoldingRanges`, `Parity.StructuralLSP.Coverage`, `ST-002`, `FlavorLSP.Navigation.ProfileResolution` | Phase 17, Phase 25, Phase 26, Phase 27, Phase 28, Phase 29, Phase 30, Phase 31 |
| `src/handlers/__tests__/semantic-tokens.handler.test.ts` | Unit | Tests semantic-token encoding for OFM tokens plus GFM task/strikethrough markers, GLFM markers/footnotes, Pandoc citations/footnotes/attributes, MultiMarkdown metadata/labels/citations/footnotes, MDX ESM/JSX/expression tokens, kramdown attribute/footnote tokens, and Markdown Extra attribute/footnote/abbreviation tokens | `FlavorLSP.SemanticTokens.ProfileTokens` | Phase 12, Phase 25, Phase 26, Phase 27, Phase 28, Phase 29, Phase 30, Phase 31 |
...
| `src/test/integration/markdown-flavor.test.ts` | Integration | Tests spawned-server Markdown flavor propagation, open-document refresh, project TOML evidence, unsupported flavor rejection, Original/CommonMark parser-diagnostic-completion behavior, Obsidian parser/diagnostic behavior, GFM parser/diagnostic counts, GLFM parser/diagnostic counts, Pandoc parser/diagnostic counts, MultiMarkdown parser/diagnostic counts, MDX parser/diagnostic counts, kramdown parser/diagnostic counts, Markdown Extra parser/diagnostic counts, and host/bibliography/conversion/renderer-boundary classification | `Extension.MarkdownFlavor.ServerPropagation`, `Extension.MarkdownFlavor.Refresh`, `FlavorLSP.Parser.ProfileDispatch`, `FlavorLSP.Diagnostics.ProfileRules`, `FlavorLSP.Completion.ProfileCandidates`, `FlavorLSP.HostBoundary.NonLocalReferences`, `Security.Input.ProjectConfigTOMLSafety` | Phase 20, Phase 22, Phase 23, Phase 24, Phase 25, Phase 26, Phase 27, Phase 28, Phase 29, Phase 30, Phase 31 |
