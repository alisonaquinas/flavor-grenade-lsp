...
|---|---|---|---|
| Unit | Pure flavor enum/schema, selector state, auto-detection resolver, syntax inference fixture inventory, structured-profile fixture inventory, `.fgattributes` target choice, refresh triggers, dialect profile registry, contribution scoping, server configuration handling, and per-LSP-surface fixture expectations. | `extension/src/markdown-flavor.test.ts`, `extension/src/markdown-flavor-evidence.test.ts`, `extension/src/client-options.test.ts`, `src/parser/__tests__/markdown-flavor-profiles.test.ts`, `src/parser/__tests__/markdown-flavor-parser-analysis.test.ts`, `src/lsp/handlers/__tests__/configuration.handler.test.ts`, `extension/test/contributions/*.test.ts` | ✅ profile registry, server propagation, and fixture inventory implemented; syntax inference and structured-profile implementation remain planned |
