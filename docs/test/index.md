...
| `src/lsp/handlers/__tests__/configuration.handler.test.ts` | Unit | Tests server Markdown flavor configuration validation, `.fgignore`/`.fgattributes` resolution, Auto Detect, open-document refresh, parser profile dispatch, and host-boundary classification | `Extension.MarkdownFlavor.ServerPropagation`, `Extension.MarkdownFlavor.AutoDetection`, `Extension.MarkdownFlavor.Refresh`, `FlavorLSP.Parser.ProfileDispatch`, `FlavorLSP.HostBoundary.NonLocalReferences`, `Security.Input.FlavorPropagationPayload`, `Security.Input.ProjectConfigSafety`, `Security.Vault.ProjectConfigConfinement` | Phase 20 |
...
| `src/test/integration/markdown-flavor.test.ts` | Integration | Tests spawned-server Markdown flavor propagation, open-document refresh, `.fgignore`/`.fgattributes` evidence, unsupported flavor rejection, Auto Detect independence, Original/CommonMark parser-diagnostic-completion behavior, Obsidian parser/diagnostic behavior, GFM parser/diagnostic counts, GLFM parser/diagnostic counts, Pandoc parser/diagnostic counts, MultiMarkdown parser/diagnostic counts, MDX parser/diagnostic counts, kramdown parser/diagnostic counts, Markdown Extra parser/diagnostic counts, R Markdown parser/diagnostic counts, Reddit parser/diagnostic counts, Stack Overflow parser/diagnostic counts, and host/bibliography/conversion/renderer/execution-boundary classification | `Extension.MarkdownFlavor.ServerPropagation`, `Extension.MarkdownFlavor.Refresh`, `FlavorLSP.Parser.ProfileDispatch`, `FlavorLSP.Diagnostics.ProfileRules`, `FlavorLSP.Completion.ProfileCandidates`, `FlavorLSP.HostBoundary.NonLocalReferences`, `Security.Input.ProjectConfigSafety` | Phase 20, Phase 22, Phase 23, Phase 24, Phase 25, Phase 26, Phase 27, Phase 28, Phase 29, Phase 30, Phase 31, Phase 32, Phase 33, Phase 34 |
...
| `src/lsp/handlers/__tests__/configuration.handler.test.ts` | Unit | Tests effective Markdown flavor payloads propagate to server analysis, refresh open documents, validate `.fgignore`/`.fgattributes` and resource payloads, and classify host boundaries | `Extension.MarkdownFlavor.ServerPropagation`, `Extension.MarkdownFlavor.Refresh`, `FlavorLSP.HostBoundary.NonLocalReferences`, `Security.Input.FlavorPropagationPayload`, `Security.Input.ProjectConfigSafety` | Phase 20 | ✅ implemented |
