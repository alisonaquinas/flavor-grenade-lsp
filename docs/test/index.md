...
| `src/parser/__tests__/markdown-flavor-parser-analysis.test.ts` | Unit | Tests Original Markdown parser/profile behavior for setext headings, core syntax, inactive Obsidian constructs, and Phase 22 surface status | `Extension.MarkdownFlavor.DialectProfiles`, `FlavorLSP.Profile.SignatureCoverage`, `FlavorLSP.Parser.ProfileDispatch` | Phase 22 |
...
| `src/resolution/__tests__/diagnostic-service.test.ts` | Unit | Tests wiki-link, block-reference, embed, and Original Markdown portability diagnostics, including FG101 warnings for inactive Original Markdown extensions | `Diagnostic.Severity.WikiLink`, `Diagnostic.Severity.Embed`, `FlavorLSP.Diagnostics.ProfileRules` | Phase 8, Phase 22 |
...
| `src/completion/__tests__/completion-router.test.ts` | Unit | Tests Markdown document and heading completion routing, external URL suppression, nested source path relativity, attachment completion ranking, and Original Markdown suppression for inactive Obsidian completions | `Completion.Trigger.Coverage`, `Parity.MarkdownLinks.Completion`, `Parity.Attachments.Completion`, `Parity.Attachments.ConfigHints`, `FlavorLSP.Completion.ProfileCandidates` | Phase 14, Phase 22 |
...
| `src/test/integration/markdown-flavor.test.ts` | Integration | Tests spawned-server Markdown flavor propagation, open-document refresh, project TOML evidence, unsupported flavor rejection, Original Markdown parser/diagnostic/completion behavior, and host-boundary classification | `Extension.MarkdownFlavor.ServerPropagation`, `Extension.MarkdownFlavor.Refresh`, `FlavorLSP.Parser.ProfileDispatch`, `FlavorLSP.Diagnostics.ProfileRules`, `FlavorLSP.Completion.ProfileCandidates`, `FlavorLSP.HostBoundary.NonLocalReferences`, `Security.Input.ProjectConfigTOMLSafety` | Phase 20, Phase 22 |
