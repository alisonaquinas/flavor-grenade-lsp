...
| `src/parser/__tests__/markdown-flavor-parser-analysis.test.ts` | Unit | Tests Original Markdown, CommonMark, Obsidian, GFM, GLFM, Pandoc, MultiMarkdown, MDX, kramdown, Markdown Extra, R Markdown, Reddit, and Stack Overflow parser/profile behavior for headings, core syntax, active/inactive extension constructs, autolinks, vault syntax, GFM tables/tasks/strikethrough, GLFM task/description-list/footnote/TOC/host-reference syntax, Pandoc title/citation/footnote/attribute/fenced-Div/definition-list syntax, MultiMarkdown metadata/table/footnote/citation/label/cross-reference/abbreviation syntax, MDX ESM/JSX/expression regions, kramdown attributes/definition-lists/tables/footnotes/math blocks, Markdown Extra tables/definition-lists/footnotes/abbreviations/fenced-code/attributes, R Markdown metadata/chunks/options/inline markers, Reddit spoilers/superscripts/strikethrough/tables/host references/portability markers, Stack Overflow tag references/spoilers/language directives/fence hints/tables/portability markers, opaque regions, and implemented surface status | `Extension.MarkdownFlavor.DialectProfiles`, `FlavorLSP.Profile.SignatureCoverage`, `FlavorLSP.Parser.ProfileDispatch` | Phase 22, Phase 23, Phase 24, Phase 25, Phase 26, Phase 27, Phase 28, Phase 29, Phase 30, Phase 31, Phase 32, Phase 33, Phase 34 |
| `src/lsp/handlers/__tests__/configuration.handler.test.ts` | Unit | Tests server Markdown flavor configuration validation, resource payload propagation, Auto Detect, open-document refresh, parser profile dispatch, and host-boundary classification; `.mdfignore`/`.mdfattributes` resolution coverage is planned | `Extension.MarkdownFlavor.ServerPropagation`, `Extension.MarkdownFlavor.AutoDetection`, `Extension.MarkdownFlavor.Refresh`, `FlavorLSP.Parser.ProfileDispatch`, `FlavorLSP.HostBoundary.NonLocalReferences`, `Security.Input.FlavorPropagationPayload`, `Security.Input.ProjectConfigSafety`, `Security.Vault.ProjectConfigConfinement` | Phase 20 |
...
| `src/resolution/__tests__/diagnostic-service.test.ts` | Unit | Tests wiki-link, block-reference, embed, Original FG101 portability diagnostics, CommonMark FG102 portability diagnostics, active Obsidian diagnostic behavior, GFM malformed-table diagnostics, GLFM malformed description-list diagnostics, Pandoc malformed-attribute diagnostics, MultiMarkdown malformed-metadata diagnostics, MDX malformed-boundary diagnostics, kramdown malformed-attribute diagnostics, Markdown Extra malformed-attribute diagnostics, R Markdown malformed-chunk diagnostics, Reddit portability diagnostics, and Stack Overflow malformed-language-directive diagnostics | `Diagnostic.Severity.WikiLink`, `Diagnostic.Severity.Embed`, `FlavorLSP.Diagnostics.ProfileRules` | Phase 8, Phase 22, Phase 23, Phase 24, Phase 25, Phase 26, Phase 27, Phase 28, Phase 29, Phase 30, Phase 31, Phase 32, Phase 33, Phase 34 |
...
| `src/completion/__tests__/completion-router.test.ts` | Unit | Tests Markdown document and heading completion routing, external URL suppression, nested source path relativity, attachment completion ranking, non-Obsidian suppression for inactive Obsidian completions, active Obsidian completion routing, GFM table/task snippets, GLFM inapplicable-task/TOC snippets, Pandoc citation/attribute snippets, MultiMarkdown metadata/citation/footnote snippets, MDX component/expression/export snippets, kramdown attribute/footnote snippets, Markdown Extra table/footnote/abbreviation/attribute snippets, R Markdown chunk/option/inline snippets, Reddit spoiler/superscript snippets, and Stack Overflow tag-reference/language-directive snippets | `Completion.Trigger.Coverage`, `Parity.MarkdownLinks.Completion`, `Parity.Attachments.Completion`, `Parity.Attachments.ConfigHints`, `FlavorLSP.Completion.ProfileCandidates` | Phase 14, Phase 22, Phase 23, Phase 24, Phase 25, Phase 26, Phase 27, Phase 28, Phase 29, Phase 30, Phase 31, Phase 32, Phase 33, Phase 34 |
...
| `src/handlers/__tests__/document-symbol.handler.test.ts` | Unit | Tests document symbols for headings, block anchors, GFM tables/tasks, GLFM description lists/TOC tags, Pandoc metadata/labels/footnotes, MultiMarkdown metadata/labels/citations/footnotes, MDX ESM/JSX/expression symbols, kramdown attribute/definition-list/table/footnote symbols, Markdown Extra attribute/definition-list/table/footnote/abbreviation symbols, R Markdown chunk/inline symbols, Reddit table/host-reference symbols, and Stack Overflow table/tag-reference symbols | `Parity.StructuralLSP.Coverage`, `FlavorLSP.Navigation.ProfileResolution` | Phase 17, Phase 25, Phase 26, Phase 27, Phase 28, Phase 29, Phase 30, Phase 31, Phase 32, Phase 33, Phase 34 |
| `src/handlers/__tests__/folding-range.handler.test.ts` | Unit | Tests structural folding ranges for frontmatter, headings, callouts, opaque code, math, comments, Templater regions, GFM table blocks, GLFM description lists, Pandoc fenced Divs/definition lists, MultiMarkdown metadata/table blocks, MDX JSX/expression blocks, kramdown definition-list/table/math blocks, Markdown Extra definition-list/table/fenced-code blocks, R Markdown chunks, Reddit table blocks, and Stack Overflow table blocks | `Parity.StructuralLSP.FoldingRanges`, `Parity.StructuralLSP.Coverage`, `ST-002`, `FlavorLSP.Navigation.ProfileResolution` | Phase 17, Phase 25, Phase 26, Phase 27, Phase 28, Phase 29, Phase 30, Phase 31, Phase 32, Phase 33, Phase 34 |
| `src/handlers/__tests__/semantic-tokens.handler.test.ts` | Unit | Tests semantic-token encoding for OFM tokens plus GFM task/strikethrough markers, GLFM markers/footnotes, Pandoc citations/footnotes/attributes, MultiMarkdown metadata/labels/citations/footnotes, MDX ESM/JSX/expression tokens, kramdown attribute/footnote tokens, Markdown Extra attribute/footnote/abbreviation tokens, R Markdown engine/label/option/inline tokens, Reddit spoiler/superscript/host-reference tokens, and Stack Overflow tag/language/spoiler tokens | `FlavorLSP.SemanticTokens.ProfileTokens` | Phase 12, Phase 25, Phase 26, Phase 27, Phase 28, Phase 29, Phase 30, Phase 31, Phase 32, Phase 33, Phase 34 |
...
| `src/test/integration/markdown-flavor.test.ts` | Integration | Tests spawned-server Markdown flavor propagation, open-document refresh, unsupported flavor rejection, Original/CommonMark parser-diagnostic-completion behavior, Obsidian parser/diagnostic behavior, GFM parser/diagnostic counts, GLFM parser/diagnostic counts, Pandoc parser/diagnostic counts, MultiMarkdown parser/diagnostic counts, MDX parser/diagnostic counts, kramdown parser/diagnostic counts, Markdown Extra parser/diagnostic counts, R Markdown parser/diagnostic counts, Reddit parser/diagnostic counts, Stack Overflow parser/diagnostic counts, and host/bibliography/conversion/renderer/execution-boundary classification; `.mdfignore`/`.mdfattributes` evidence and Auto Detect independence coverage is planned | `Extension.MarkdownFlavor.ServerPropagation`, `Extension.MarkdownFlavor.Refresh`, `FlavorLSP.Parser.ProfileDispatch`, `FlavorLSP.Diagnostics.ProfileRules`, `FlavorLSP.Completion.ProfileCandidates`, `FlavorLSP.HostBoundary.NonLocalReferences`, `Security.Input.ProjectConfigSafety` | Phase 20, Phase 22, Phase 23, Phase 24, Phase 25, Phase 26, Phase 27, Phase 28, Phase 29, Phase 30, Phase 31, Phase 32, Phase 33, Phase 34 |
...
> launches VS Code and runs all host fixtures: `.obsidian/`, Flavor Grenade
> `.mdfignore`/`.mdfattributes` markers, and generic Markdown.

### Markdown Flavor Test Plan By Level

| Level | Planned coverage | Primary files | Status |
|---|---|---|---|
| Unit | Pure flavor enum/schema, selector state, auto-detection resolver, syntax inference fixture inventory, structured-profile fixture inventory, `.mdfattributes` target choice, refresh triggers, dialect profile registry, contribution scoping, server configuration handling, and per-LSP-surface fixture expectations. | `extension/src/markdown-flavor.test.ts`, `extension/src/markdown-flavor-evidence.test.ts`, `extension/src/client-options.test.ts`, `src/parser/__tests__/markdown-flavor-profiles.test.ts`, `src/parser/__tests__/markdown-flavor-parser-analysis.test.ts`, `src/lsp/handlers/__tests__/configuration.handler.test.ts`, `extension/test/contributions/*.test.ts` | ✅ profile registry, server propagation, and fixture inventory implemented; syntax inference and structured-profile implementation remain planned |
| Integration | Spawned-server or multi-module tests that prove effective flavor and structured-profile flags reach parser, diagnostics, completion, navigation, hover, semantic tokens, rename, host-boundary classification, and open-document refresh. | `src/test/integration/markdown-flavor.test.ts`, `src/test/integration/transport.test.ts`, `src/test/integration/navigation.test.ts` | ✅ server propagation implemented; per-surface dialect and structured-profile fixtures planned |
| E2E | Root BDD specs define acceptance state; VS Code Extension Development Host scenarios must prove visible selector behavior, quick-pick choices, selected-file/directory `.mdfattributes` persistence, structured-profile flags, language preservation, and server refresh from real extension wiring. | `docs/bdd/features/ofmarkdown-language-mode.feature`, `docs/bdd/features/markdown-flavor-dialects.feature`, `extension/src/test/suite/markdown-flavor.test.js`, `extension/src/test/suite/index.js` | 🟡 root BDD updated with planned config-file scenarios; extension host planned |
...
| Validation | Acceptance-level evidence that the researched flavor list, dialect profiles, structured profiles, and non-local host-boundary dispositions match product requirements and source research. | `docs/bdd/features/ofmarkdown-language-mode.feature`, `docs/bdd/features/markdown-flavor-dialects.feature`, `docs/research/*.md`, `docs/test/evidence/markdown-flavor-*.md`, `docs/test/evidence/markdown-structured-profile-review.md`, `extension/docs/tests/evidence/markdown-flavor-*.md`, `extension/docs/tests/evidence/markdown-structured-profile-smoke.md` | 🟡 root flavor validation implemented; structured-profile and extension validation planned |
...
| `extension/src/markdown-flavor-evidence.test.ts` | Unit | Fixture inventory for `.mdfattributes`-configured flavor smoketests, config-absent inference samples, structured-profile examples, ambiguous fallback samples, and root README boundary-negative behavior | `Extension.MarkdownFlavor.AutoDetection`, `Extension.MarkdownStructuredProfiles.Configuration`, `FlavorLSP.StructuredProfiles.Flags`, `Security.Vault.ProjectConfigConfinement` | Markdown flavor requirements | ✅ partial |
...
| `src/lsp/handlers/__tests__/configuration.handler.test.ts` | Unit | Tests effective Markdown flavor payloads propagate to server analysis, refresh open documents, validate resource payloads, and classify host boundaries; `.mdfignore`/`.mdfattributes` validation is planned | `Extension.MarkdownFlavor.ServerPropagation`, `Extension.MarkdownFlavor.Refresh`, `FlavorLSP.HostBoundary.NonLocalReferences`, `Security.Input.FlavorPropagationPayload`, `Security.Input.ProjectConfigSafety` | Phase 20 | ✅ implemented for payloads; config-file update planned |
...
| `extension/test/package-targets/server-binary.test.ts` | Unit | Tests packaged server module validation, native executable rejection, missing/duplicate payload rejection, and real VSIX archive inspection | `Extension.Packaging.ServerModuleValidation` | Phase E14 | ✅ implemented |
...
| `extension/src/test/suite/activation-language-mode.test.js` | Integration | Legacy host tests for vault startup, legacy config startup, generic Markdown idle behavior, OFMarkdown promotion, and manual non-Markdown preservation; replace with Markdown flavor host coverage for `.mdfignore`/`.mdfattributes` activation and selector state | `Extension.Activation.VaultPrecision`, `Extension.MarkdownFlavor.Refresh`, `Extension.Tests.HostCoverage` | Phase E9 | 🔴 obsolete |
| `extension/src/test/suite/markdown-flavor.test.js` | Integration | Planned host tests for selector UI, required flavor choices, selected-file/directory `.mdfattributes` persistence, `.mdfignore` inactive state, generic CommonMark fallback, Obsidian auto-detection, syntax inference, boundary-negative fixtures, and manual-language safety | `Extension.MarkdownLanguage.PreserveDefault`, `Extension.MarkdownFlavor.Selector`, `Extension.MarkdownFlavor.RequiredCoverage`, `Extension.MarkdownFlavor.AutoDetection`, `Extension.MarkdownFlavor.OverridePersistence`, `Extension.MarkdownFlavor.Refresh`, `Extension.Tests.HostCoverage` | Markdown flavor requirements | 📋 planned |
...
| `docs/bdd/features/ofmarkdown-language-mode.feature` | `src/test/bdd/step-definitions/extension-harness.steps.ts` | Acceptance scenario groups covering Markdown language preservation, flavor selection, override persistence, auto-detection, syntax inference, boundary confinement, and manual-language safety | Markdown flavor requirements | 🟡 needs inference update |
