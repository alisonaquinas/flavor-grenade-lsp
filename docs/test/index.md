...
| `src/lsp/handlers/__tests__/configuration.handler.test.ts` | Unit | Tests server Markdown flavor configuration validation, resource payload propagation, Auto Detect, open-document refresh, parser profile dispatch, and host-boundary classification; `.fgignore`/`.fgattributes` resolution coverage is planned | `Extension.MarkdownFlavor.ServerPropagation`, `Extension.MarkdownFlavor.AutoDetection`, `Extension.MarkdownFlavor.Refresh`, `FlavorLSP.Parser.ProfileDispatch`, `FlavorLSP.HostBoundary.NonLocalReferences`, `Security.Input.FlavorPropagationPayload`, `Security.Input.ProjectConfigSafety`, `Security.Vault.ProjectConfigConfinement` | Phase 20 |
...
> `.fgignore`/`.fgattributes` markers, and generic Markdown.

### Markdown Flavor Test Plan By Level

| Level | Planned coverage | Primary files | Status |
|---|---|---|---|
| Unit | Pure flavor enum/schema, selector state, auto-detection resolver, syntax/context inference fixture inventory, structured-profile fixture inventory, `.fgattributes` target choice, refresh triggers, dialect profile registry, contribution scoping, server configuration handling, and per-LSP-surface fixture expectations. | `extension/src/markdown-flavor.test.ts`, `extension/src/markdown-flavor-evidence.test.ts`, `extension/src/client-options.test.ts`, `src/parser/__tests__/markdown-flavor-profiles.test.ts`, `src/parser/__tests__/markdown-flavor-parser-analysis.test.ts`, `src/lsp/handlers/__tests__/configuration.handler.test.ts`, `extension/test/contributions/*.test.ts` | ✅ profile registry, server propagation, and fixture inventory implemented; syntax/context inference and structured-profile implementation remain planned |
...
| `extension/src/markdown-flavor-evidence.test.ts` | Unit | Fixture inventory for `.fgattributes`-configured flavor smoketests, config-absent inference samples, structured-profile examples, ambiguous fallback samples, and root README boundary-negative behavior | `Extension.MarkdownFlavor.AutoDetection`, `Extension.MarkdownStructuredProfiles.Configuration`, `FlavorLSP.StructuredProfiles.Flags`, `Security.Vault.ProjectConfigConfinement` | Markdown flavor requirements | ✅ partial |
...
| `src/lsp/handlers/__tests__/configuration.handler.test.ts` | Unit | Tests effective Markdown flavor payloads propagate to server analysis, refresh open documents, validate resource payloads, and classify host boundaries; `.fgignore`/`.fgattributes` validation is planned | `Extension.MarkdownFlavor.ServerPropagation`, `Extension.MarkdownFlavor.Refresh`, `FlavorLSP.HostBoundary.NonLocalReferences`, `Security.Input.FlavorPropagationPayload`, `Security.Input.ProjectConfigSafety` | Phase 20 | ✅ implemented for payloads; config-file update planned |
