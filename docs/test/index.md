...
|---|---|---|---|
| Unit | Pure flavor enum/schema, selector state, auto-detection resolver, syntax/context inference fixture inventory, settings target choice, refresh triggers, dialect profile registry, contribution scoping, server configuration handling, and per-LSP-surface fixture expectations. | `extension/src/markdown-flavor.test.ts`, `extension/src/markdown-flavor-evidence.test.ts`, `extension/src/client-options.test.ts`, `src/parser/__tests__/markdown-flavor-profiles.test.ts`, `src/parser/__tests__/markdown-flavor-parser-analysis.test.ts`, `src/lsp/handlers/__tests__/configuration.handler.test.ts`, `extension/test/contributions/*.test.ts` | ✅ profile registry, server propagation, and fixture inventory implemented; syntax/context inference implementation remains planned |
...
| `extension/src/markdown-flavor-evidence.test.ts` | Unit | Fixture inventory for configured flavor smoketests, TOML-absent inference samples, ambiguous fallback samples, and root README boundary-negative behavior | `Extension.MarkdownFlavor.AutoDetection`, `Security.Vault.ProjectConfigConfinement` | Markdown flavor requirements | ✅ partial |
