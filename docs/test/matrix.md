...
| `Extension.MarkdownFlavor.OverridePersistence` | Folder-backed overrides persist to project settings; standalone overrides persist to user settings | `docs/bdd/features/ofmarkdown-language-mode.feature`, planned `extension/src/markdown-flavor.test.ts`, planned `extension/src/test/suite/markdown-flavor.test.js` | 🔴 failing | Markdown flavor requirements | Needs VS Code configuration-target tests for workspace-folder/workspace/user scope |
| `Extension.MarkdownFlavor.ServerPropagation` | Effective Markdown flavor propagates to server analysis | `docs/bdd/features/ofmarkdown-language-mode.feature`, planned `extension/src/markdown-flavor.test.ts`, planned `src/lsp/handlers/__tests__/configuration.handler.test.ts` | 🔴 failing | Markdown flavor requirements | Needs configuration or document-metadata refresh tests across all required flavor ids |
