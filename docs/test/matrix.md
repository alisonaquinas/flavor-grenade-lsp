...
| Planguage Tag | Requirement Gist | Test File(s) | Status | Phase | Notes |
|---|---|---|---|---|---|
| `CICD.Workflow.PRGate` | Every PR must pass all CI checks before merge | `.github/workflows/ci.yml`, `src/test/ci-workflow.test.ts` | ✅ passing | Phase 13, Phase 18 | TASK-282 expands PR CI to root tests, BDD scenarios, extension checks, website checks, docs lint, dependency policy, format, and build |
...
| `Extension.Tests.HostCoverage` | Extension-host tests cover required client behavior groups | `.github/workflows/ci.yml`, `src/test/ci-workflow.test.ts`, `extension/src/test/suite/*.js`, `docs/bdd/features/ofmarkdown-language-mode.feature`, `docs/bdd/features/markdown-flavor-dialects.feature` | 🔴 failing | Markdown flavor requirements | Existing host gate runs, but required Markdown flavor selector, override persistence, and researched-flavor fixtures are not implemented yet |
...
| `Extension.MarkdownFlavor.DialectProfiles` | Every supported explicit flavor has a source-backed dialect profile | `docs/bdd/features/markdown-flavor-dialects.feature`, planned `src/parser/__tests__/markdown-flavor-profiles.test.ts` | 🔴 failing | Markdown flavor requirements | BDD examples exist; implementation needs profile registry/parser behavior coverage |
| `Extension.MarkdownFlavor.AutoDetection` | Auto Detect infers flavor from vault and workspace signals | `docs/bdd/features/ofmarkdown-language-mode.feature`, `src/vault/__tests__/document-membership.test.ts`, planned `extension/src/markdown-flavor.test.ts` | 🔴 failing | Markdown flavor requirements | Existing membership tests are reusable evidence, but extension-side effective flavor resolution is not implemented |
...
| `Extension.MarkdownFlavor.ServerPropagation` | Effective Markdown flavor propagates to server analysis | `docs/bdd/features/ofmarkdown-language-mode.feature`, planned `extension/src/markdown-flavor.test.ts`, planned `src/lsp/handlers/__tests__/configuration.handler.test.ts` | 🔴 failing | Markdown flavor requirements | BDD asserts recorded `workspace/didChangeConfiguration` payloads; product tests must verify real client/server propagation across all required flavor ids |
...
| `FlavorLSP.HostBoundary.NonLocalReferences` | Host and conversion references stay separate from local vault/file/heading/label/citation targets | planned `src/test/integration/markdown-flavor.test.ts`; `docs/test/evidence/markdown-flavor-host-boundary-review.md`; `docs/plans/markdown-flavor-lsp-applicability-matrix.md` | 🔴 failing | Phase 20, Phase 21, and Phases 25-34 | Phase 20 owns the shared classifier; Phase 25 adds GitHub host-reference coverage; later dialect phases add boundary fixtures and deferred lookup dispositions. |
| `Security.Parser.FlavorProfileResourceSafety` | Flavor parsers inherit resource-safety, timeout, and ReDoS gates | planned `src/parser/__tests__/markdown-flavor-profiles.test.ts`; planned pathological flavor fixtures | 🔴 failing | Phase 19 and Phases 22-34 | Profile registry records parser safety metadata; each dialect phase adds pathological parser fixtures before closing. |
| `Security.Input.ProjectConfigTOMLSafety` | `.flavor-grenade.toml` is confined, size-limited, schema-validated, and log-redacted | planned server config tests; planned integration TOML fixtures | 🔴 failing | Phase 20 | Project flavor evidence cannot affect effective flavor until TOML path, size, type, and dangerous-key checks pass. |
| `Security.Input.FlavorPropagationPayload` | Resource-specific flavor payloads reject oversized maps, unsafe URIs, dangerous keys, stale resources, and invalid effective values | planned server and extension propagation tests | 🔴 failing | Phase 20 and Phase E15 | Client and server both validate payload shape before state mutation or propagation. |
| `Security.Vault.ProjectConfigConfinement` | Project config discovery reads only in-root realpaths | planned project-config fixtures | 🔴 failing | Phase 20 | Symlinked or out-of-root TOML is treated as absent configuration. |
...
| Validation | `Extension.MarkdownFlavor.RequiredCoverage`, `Extension.MarkdownFlavor.DialectProfiles`, `FlavorLSP.Profile.SignatureCoverage`, `FlavorLSP.HostBoundary.NonLocalReferences`, validation evidence redaction | `docs/bdd/features/ofmarkdown-language-mode.feature`, `docs/bdd/features/markdown-flavor-dialects.feature`, `docs/test/evidence/markdown-flavor-research-trace.md`, `docs/test/evidence/markdown-flavor-product-review.md`, `docs/test/evidence/markdown-flavor-validation-run.md`, `docs/test/evidence/markdown-flavor-host-boundary-review.md` | 🔴 needs step updates | Confirms the supported flavor list, server profile claims, non-local boundary dispositions, and sanitized evidence match `docs/research/`, feature pages, and ADR020. |
...
| `docs/test/evidence/markdown-flavor-host-boundary-review.md` | `FlavorLSP.HostBoundary.NonLocalReferences`, `Security.Vault.PathConfinement` | Phase 20 shared classifier; Phase 21 scaffold; Phases 25-34 updates | 🔴 failing | Must record shared classifier evidence, per-platform/conversion false-local-resolution checks, and deferred lookup dispositions. |
