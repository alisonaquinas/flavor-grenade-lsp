...
| `Extension.Workspace.EnvironmentModes` | Restricted, virtual, local, and remote workspace modes have explicit startup behavior and smoke evidence | `extension/src/workspace-environment.test.ts`, `extension/src/extension-startup.test.ts`, `extension/docs/features/workspace-environments.md` | 🔴 failing | Phase E13, Phase 18 | Startup guard regression test added for BUG-025; it fails until the shared command start path blocks unsupported environments before server spawn |
