...
| `Security.Vault.ProjectConfigConfinement` | `.fgignore` and `.fgattributes` discovery reads only in-root realpaths | `src/lsp/handlers/__tests__/configuration.handler.test.ts`; planned config-file discovery tests | 🔴 needs update | Phase 20 | Config-file discovery must use realpath vault confinement before file I/O; unsafe candidates are treated as absent configuration without logging content. |
