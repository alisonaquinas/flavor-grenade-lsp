...
| `Workspace.VaultDetection.Fallback` | Directories with Markdown flavor config-file markers detected when `.obsidian/` absent | `src/vault/__tests__/vault-detector.test.ts`; `docs/bdd/features/vault-detection.feature` | ✅ passing | Phase 35 | Coverage proves `.mdfignore` and `.mdfattributes` marker context for non-Obsidian Markdown roots. Legacy `.flavor-grenade.*` files remain operational config only and no longer act as flavor/vault markers. |
...
| `Security.Config.NoCodeExecution` | Project config schemas never include command/script/executable fields; no process spawning | — | ⏳ planned | Phase 1 | Schema inspection + crafted-config integration test; see ADR012 |
...
| `Extension.MarkdownLanguage.PreserveDefault` | `.md` documents stay in VS Code's built-in `markdown` language mode | `docs/bdd/features/ofmarkdown-language-mode.feature`, `docs/bdd/features/vscode-extension.feature`, `extension/src/language-mode.test.ts`, planned `extension/src/test/suite/markdown-flavor.test.js` | 🟡 unit passing; host pending | Markdown flavor requirements | Unit coverage proves the controller refreshes flavor state without switching language id; VS Code host proof remains planned. |
| `Extension.MarkdownFlavor.Selector` | Extension exposes a separate Markdown flavor selector | `docs/bdd/features/ofmarkdown-language-mode.feature`, `extension/src/markdown-flavor.test.ts`, planned `extension/src/test/suite/markdown-flavor.test.js` | 🟡 unit passing; host pending | Markdown flavor requirements | Unit coverage proves selector constants, quick-pick choices, and second scope prompt options; host quick-pick automation remains planned. |
| `Extension.MarkdownFlavor.RequiredCoverage` | Selector choices, `.mdfattributes` accepted enum, and server-facing model include every researched Markdown flavor | `docs/bdd/features/ofmarkdown-language-mode.feature`, `docs/bdd/features/markdown-flavor-dialects.feature`, `extension/src/markdown-flavor.test.ts`, `extension/src/markdown-flavor-evidence.test.ts`, `src/markdown-flavor/__tests__/mdf-config-files.test.ts` | ✅ passing | Markdown flavor requirements, Phase 35 | Tests cover `auto`, `original`, `commonmark`, `obsidian`, `gfm`, `glfm`, `pandoc`, `multimarkdown`, `mdx`, `kramdown`, `markdown-extra`, `r-markdown`, `reddit`, and `stack-overflow` through selector constants plus `.mdfattributes` parser/evidence paths. |
...
| `Extension.MarkdownFlavor.AutoDetection` | Auto Detect infers flavor from Obsidian markers and strong syntax evidence after config resolution requests auto | `docs/bdd/features/ofmarkdown-language-mode.feature`, `src/markdown-flavor/__tests__/markdown-flavor-state.test.ts`, `extension/src/markdown-flavor.test.ts`, `extension/src/markdown-flavor-evidence.test.ts` | ✅ passing | Markdown flavor requirements, Phase 35 | Server and extension tests prove `.mdfattributes` concrete selection is configuration, while `flavor=auto`, `!flavor`, absent config, Obsidian markers, syntax inference, and CommonMark fallback stay in the independent Auto Detect path. |
| `Extension.MarkdownStructuredProfiles.Configuration` | Extension configures Keep a Changelog, Common Changelog, and MADR as independent profile flags | `extension/src/markdown-flavor-evidence.test.ts`, `extension/src/markdown-flavor.test.ts`, `extension/docs/tests/markdown-flavor-unit-spec.md`, `extension/docs/tests/markdown-flavor-integration-spec.md` | 🟡 unit passing; host pending | Structured profile requirements | Fixture inventory and extension tests cover both changelog variants and MADR under `.mdfattributes`-configured and config-absent inference smoke workspaces. Host UI proof remains planned. |
| `Extension.MarkdownFlavor.OverridePersistence` | Selector choices persist through selected-file and directory `.mdfattributes` rules | `docs/bdd/features/ofmarkdown-language-mode.feature`, `extension/src/markdown-flavor.test.ts`, planned `extension/src/test/suite/markdown-flavor.test.js` | 🟡 unit passing; host pending | Markdown flavor requirements, Phase 35 | BDD and unit tests cover selected-file rules, `All Markdown files in this directory`, same-scope Auto Detect reset with `!flavor`, and `.mdfignore` inactive write blocking. VS Code host write proof remains planned. |
| `Extension.MarkdownFlavor.ServerPropagation` | Effective Markdown flavor propagates to server analysis | `docs/bdd/features/ofmarkdown-language-mode.feature`, `src/lsp/handlers/__tests__/configuration.handler.test.ts`, `src/test/integration/markdown-flavor.test.ts`, `extension/src/markdown-flavor.test.ts`, `extension/src/language-mode.test.ts` | ✅ passing | Markdown flavor requirements, Phase 35 | Extension now sends refresh-only configuration notifications; spawned-server tests prove `.mdfattributes` changes refresh effective parse context and legacy flavor payloads no longer override config files. |
| `Extension.MarkdownFlavor.ManualLanguageSafety` | Manual non-Markdown language selections are preserved | `docs/bdd/features/ofmarkdown-language-mode.feature`, `extension/src/markdown-flavor.test.ts`, `extension/src/language-mode.test.ts`, planned `extension/src/test/suite/markdown-flavor.test.js` | 🟡 unit passing; host pending | Markdown flavor requirements | Tests preserve `plaintext`, `mdx`, and other user-selected language ids while still allowing `mdx` as a Markdown flavor id when language remains `markdown`; host proof remains planned. |
| `Extension.MarkdownFlavor.Refresh` | Flavor state refreshes after server, index, workspace, editor, file-open, and selector events | `docs/bdd/features/ofmarkdown-language-mode.feature`, `extension/src/markdown-flavor.test.ts`, `extension/src/language-mode.test.ts`, `src/lsp/handlers/__tests__/configuration.handler.test.ts`, `src/test/integration/markdown-flavor.test.ts` | ✅ passing | Markdown flavor requirements, Phase 35 | Tests cover refresh-only notifications, open-document refresh after config changes, marker watcher refresh hooks, and integration refresh after `.mdfattributes` changes. |
...
| Planguage Tag | Requirement Gist | Test File(s) | Status | Phase | Notes |
|---|---|---|---|---|---|
| `FlavorLSP.StructuredProfiles.Flags` | Keep a Changelog, Common Changelog, and MADR layer over any base Markdown flavor without expanding the flavor list | `docs/design/markdown-structured-profile-flags.md`; `docs/test/markdown-flavor-unit-spec.md`; `docs/test/markdown-flavor-integration-spec.md`; `extension/src/markdown-flavor-evidence.test.ts`; planned structured-profile parser/LSP tests | 🔴 failing | Structured profile requirements | Spec and fixture inventory cover both changelog variants and MADR across `.mdfattributes`-configured and config-absent inference smoke workspaces. Contract, inference, diagnostics, symbols/folds, and server propagation implementation remain pending. |
...
| `Security.Input.ProjectConfigSafety` | Flavor config files are confined, size-limited, schema-validated, and log-redacted | `src/markdown-flavor/__tests__/mdf-config-files.test.ts`; `extension/src/markdown-flavor-evidence.test.ts`; `src/test/integration/markdown-flavor.test.ts` | ✅ passing | Phase 35 | Coverage reads `.mdfignore` and `.mdfattributes` through confined roots, caps file size, validates Git-style patterns and attributes, treats invalid values as absent configuration, and proves legacy project flavor config does not remain an active file/directory flavor source. |
...
| `Security.Vault.ProjectConfigConfinement` | `.mdfignore` and `.mdfattributes` discovery reads only in-root realpaths | `src/markdown-flavor/__tests__/mdf-config-files.test.ts`; `extension/src/markdown-flavor-evidence.test.ts`; `src/vault/__tests__/vault-scanner.test.ts` | ✅ passing | Phase 35 | Config-file discovery confines reads to the vault or workspace boundary before file I/O; outside-boundary and realpath-mismatch candidates are treated as absent configuration. |
...
|---|---|---|---|---|
| Unit | `Extension.MarkdownLanguage.PreserveDefault`, `Extension.MarkdownFlavor.Selector`, `Extension.MarkdownFlavor.RequiredCoverage`, `Extension.MarkdownFlavor.AutoDetection`, `Extension.MarkdownFlavor.OverridePersistence`, `Extension.MarkdownFlavor.ServerPropagation`, `Extension.MarkdownFlavor.ManualLanguageSafety`, `Extension.MarkdownFlavor.Refresh`, `Extension.Contributions.FlavorScoped` | `extension/src/markdown-flavor.test.ts`, `extension/src/markdown-flavor-evidence.test.ts`, `extension/src/language-mode.test.ts`, `extension/test/contributions/*.test.ts`, `src/markdown-flavor/__tests__/mdf-config-files.test.ts`, `src/markdown-flavor/__tests__/markdown-flavor-state.test.ts` | ✅ passing | Unit evidence now covers `.mdfignore` visibility, `.mdfattributes` selection and negation, Auto Detect independence, propagation validation, refresh calls, selector rule writes, and language-id preservation. |
...
| Integration | `Extension.MarkdownFlavor.ServerPropagation`, `Extension.MarkdownFlavor.DialectProfiles`, `FlavorLSP.Parser.ProfileDispatch`, `FlavorLSP.Diagnostics.ProfileRules`, `FlavorLSP.Completion.ProfileCandidates` | `src/test/integration/markdown-flavor.test.ts` | ✅ passing | Spawned-server evidence proves `.mdfignore` and `.mdfattributes` reach analysis, Auto Detect remains separate from config resolution, config-file refresh updates open-document parse context, and legacy flavor payloads no longer override `.mdfattributes`. |
| E2E | `Extension.MarkdownLanguage.PreserveDefault`, `Extension.MarkdownFlavor.Selector`, `Extension.MarkdownFlavor.OverridePersistence`, `Extension.MarkdownFlavor.Refresh`, `Extension.Tests.HostCoverage` | `extension/src/test/suite/markdown-flavor.test.js` | 📋 planned | Runs through VS Code host UI plus `.mdfattributes`/`.mdfignore` surfaces with real workspace and standalone-file contexts. |
...
| Validation | `Extension.MarkdownFlavor.RequiredCoverage`, `Extension.MarkdownFlavor.DialectProfiles`, `FlavorLSP.Profile.SignatureCoverage`, `FlavorLSP.StructuredProfiles.Flags`, `FlavorLSP.HostBoundary.NonLocalReferences`, validation evidence redaction | `docs/bdd/features/ofmarkdown-language-mode.feature`, `docs/bdd/features/markdown-flavor-dialects.feature`, `docs/test/evidence/markdown-flavor-research-trace.md`, `docs/test/evidence/markdown-flavor-product-review.md`, `docs/test/evidence/markdown-flavor-validation-run.md`, `docs/test/evidence/markdown-flavor-host-boundary-review.md`, planned `docs/test/evidence/markdown-structured-profile-review.md` | 🔴 failing | Root/server validation confirms the supported base flavor list, server profile claims, non-local boundary dispositions, and sanitized evidence match existing flavor research. Structured-profile validation evidence and extension-host validation remain planned. |
...
| `docs/test/evidence/markdown-structured-profile-review.md` | `FlavorLSP.StructuredProfiles.Flags`, `Extension.MarkdownStructuredProfiles.Configuration` | Structured profile requirements | 🔴 failing | Planned evidence must trace Keep a Changelog, Common Changelog, and MADR research to fixture paths, expected profile ids, mutual-exclusion behavior, and configured/inference workspace coverage. |
