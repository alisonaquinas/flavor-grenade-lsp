...
| `Extension.MarkdownFlavor.AutoDetection` | Auto Detect infers flavor from marker, membership, and syntax/context signals after config resolution requests auto | `docs/bdd/features/ofmarkdown-language-mode.feature`, `src/vault/__tests__/document-membership.test.ts`, `extension/src/markdown-flavor-evidence.test.ts`, planned config-file resolver tests, planned `extension/src/markdown-flavor.test.ts`, planned inference classifier tests | 🔴 failing | Markdown flavor requirements, Phase 20 | Server-side config resolution must test `.fgignore` visibility, `.fgattributes` concrete selection, `flavor=auto`, `!flavor`, and absent config as inputs to Auto Detect. Auto Detect itself covers Obsidian markers/membership, syntax/context inference, and CommonMark fallback. Host proof remains pending. |
| `Extension.MarkdownStructuredProfiles.Configuration` | Extension configures Keep a Changelog, Common Changelog, and MADR as independent profile flags | `extension/src/markdown-flavor-evidence.test.ts`, planned `extension/src/markdown-flavor.test.ts`, `extension/docs/tests/markdown-flavor-unit-spec.md`, `extension/docs/tests/markdown-flavor-integration-spec.md` | 🔴 failing | Structured profile requirements | Fixture inventory now covers both changelog variants and MADR under every `.fgattributes`-configured and config-absent inference smoke workspace. `.fgattributes` parsing, host propagation, explicit overrides, and user-visible proof remain pending. |
| `Extension.MarkdownFlavor.OverridePersistence` | Selector choices persist through selected-file and directory `.fgattributes` rules | `docs/bdd/features/ofmarkdown-language-mode.feature`, planned `extension/src/markdown-flavor.test.ts`, planned `extension/src/test/suite/markdown-flavor.test.js` | 🔴 failing | Markdown flavor requirements | BDD distinguishes selected-file and `All files in this directory` scope targets; needs VS Code product tests proving `.fgattributes` writes and same-scope Auto Detect reset. |
...
| `Security.Input.ProjectConfigSafety` | Flavor config files are confined, size-limited, schema-validated, and log-redacted | planned config-file safety tests; planned config-file discovery tests; planned spawned-server config-file integration tests | 🔴 needs update | Phase 20 | New coverage must read `.fgignore` and `.fgattributes` through confined realpaths, cap file size, validate Git-style patterns and attributes, and treat dangerous keys/invalid values as absent configuration. Legacy project flavor config must not remain an active file/directory flavor source. |
...
| `Security.Vault.ProjectConfigConfinement` | `.fgignore` and `.fgattributes` discovery reads only in-root realpaths | planned config-file discovery tests | 🔴 needs update | Phase 20 | Config-file discovery must use realpath vault confinement before file I/O; unsafe candidates are treated as absent configuration without logging content. |
...
| Integration | `Extension.MarkdownFlavor.ServerPropagation`, `Extension.MarkdownFlavor.DialectProfiles`, `FlavorLSP.Parser.ProfileDispatch`, `FlavorLSP.Diagnostics.ProfileRules`, `FlavorLSP.Completion.ProfileCandidates` | `src/test/integration/markdown-flavor.test.ts` | 🟡 needs config-file update | Must prove `.fgignore` and `.fgattributes` evidence reaches spawned server analysis, Auto Detect remains separate from config resolution, diagnostics/features refresh, and Original/CommonMark/Obsidian/GFM/GLFM/Pandoc/MultiMarkdown/MDX/kramdown/Markdown Extra/R Markdown/Reddit/Stack Overflow parser, diagnostic, local count, and boundary behavior apply across JSON-RPC. |
