...
| `FlavorLSP.SemanticTokens.ProfileTokens` | Semantic tokens mark only active flavor constructs and respect opaque regions | `src/handlers/__tests__/semantic-tokens.handler.test.ts`; planned `src/test/integration/markdown-flavor.test.ts`; `docs/test/markdown-flavor-unit-spec.md` | 🔴 failing | Phases 22-34 | Phase 27 adds Pandoc citation, footnote-label, and attribute tokens. Phase 28 adds MultiMarkdown metadata, label, citation, and footnote tokens. Phase 29 adds MDX ESM declaration, JSX element, and expression tokens. Phase 30 adds kramdown attribute-marker and footnote-label tokens. Phase 31 adds Markdown Extra attribute-marker, footnote-label, and abbreviation-label tokens. Phase 32 adds R Markdown chunk-engine, chunk-label, chunk-option, and inline-expression tokens. Phase 33 adds Reddit spoiler, superscript, and host-reference tokens. Stack Overflow still requires token type/modifier expectations and inactive-syntax negative checks. |
