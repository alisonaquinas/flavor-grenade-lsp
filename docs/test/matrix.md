...
| `Extension.MarkdownFlavor.RequiredCoverage` | Selector choices, `.fgattributes` accepted enum, and server-facing model include every researched Markdown flavor | `docs/bdd/features/ofmarkdown-language-mode.feature`, `docs/bdd/features/markdown-flavor-dialects.feature`, `extension/src/markdown-flavor.test.ts`, `extension/src/markdown-flavor-evidence.test.ts`, `src/markdown-flavor/__tests__/fg-config-files.test.ts` | ✅ passing | Markdown flavor requirements, Phase 35 | Tests cover `auto`, `original`, `commonmark`, `obsidian`, `gfm`, `glfm`, `pandoc`, `multimarkdown`, `mdx`, `kramdown`, `markdown-extra`, `r-markdown`, `reddit`, and `stack-overflow` through selector constants plus `.fgattributes` parser/evidence paths. |
...
| Planguage Tag | Requirement Gist | Test File(s) | Status | Phase | Notes |
|---|---|---|---|---|---|
| `FlavorLSP.StructuredProfiles.Flags` | Keep a Changelog, Common Changelog, and MADR layer over any base Markdown flavor without expanding the flavor list | `docs/design/markdown-structured-profile-flags.md`; `docs/test/markdown-flavor-unit-spec.md`; `docs/test/markdown-flavor-integration-spec.md`; `extension/src/markdown-flavor-evidence.test.ts`; planned structured-profile parser/LSP tests | 🔴 failing | Structured profile requirements | Spec and fixture inventory cover both changelog variants and MADR across `.fgattributes`-configured and config-absent inference smoke workspaces. Contract, inference, diagnostics, symbols/folds, and server propagation implementation remain pending. |
...
| `Security.Input.ProjectConfigSafety` | Flavor config files are confined, size-limited, schema-validated, and log-redacted | `src/markdown-flavor/__tests__/fg-config-files.test.ts`; `extension/src/markdown-flavor-evidence.test.ts`; `src/test/integration/markdown-flavor.test.ts` | ✅ passing | Phase 35 | Coverage reads `.fgignore` and `.fgattributes` through confined roots, caps file size, validates Git-style patterns and attributes, treats invalid values as absent configuration, and proves legacy project flavor config does not remain an active file/directory flavor source. |
...
|---|---|---|---|---|
| Unit | `Extension.MarkdownLanguage.PreserveDefault`, `Extension.MarkdownFlavor.Selector`, `Extension.MarkdownFlavor.RequiredCoverage`, `Extension.MarkdownFlavor.AutoDetection`, `Extension.MarkdownFlavor.OverridePersistence`, `Extension.MarkdownFlavor.ServerPropagation`, `Extension.MarkdownFlavor.ManualLanguageSafety`, `Extension.MarkdownFlavor.Refresh`, `Extension.Contributions.FlavorScoped` | `extension/src/markdown-flavor.test.ts`, `extension/src/markdown-flavor-evidence.test.ts`, `extension/src/language-mode.test.ts`, `extension/test/contributions/*.test.ts`, `src/markdown-flavor/__tests__/fg-config-files.test.ts`, `src/markdown-flavor/__tests__/markdown-flavor-state.test.ts` | ✅ passing | Unit evidence now covers `.fgignore` visibility, `.fgattributes` selection and negation, Auto Detect independence, propagation validation, refresh calls, selector rule writes, and language-id preservation. |
...
| Integration | `Extension.MarkdownFlavor.ServerPropagation`, `Extension.MarkdownFlavor.DialectProfiles`, `FlavorLSP.Parser.ProfileDispatch`, `FlavorLSP.Diagnostics.ProfileRules`, `FlavorLSP.Completion.ProfileCandidates` | `src/test/integration/markdown-flavor.test.ts` | ✅ passing | Spawned-server evidence proves `.fgignore` and `.fgattributes` reach analysis, Auto Detect remains separate from config resolution, config-file refresh updates open-document parse context, and legacy flavor payloads no longer override `.fgattributes`. |
| E2E | `Extension.MarkdownLanguage.PreserveDefault`, `Extension.MarkdownFlavor.Selector`, `Extension.MarkdownFlavor.OverridePersistence`, `Extension.MarkdownFlavor.Refresh`, `Extension.Tests.HostCoverage` | `extension/src/test/suite/markdown-flavor.test.js` | 📋 planned | Runs through VS Code host UI plus `.fgattributes`/`.fgignore` surfaces with real workspace and standalone-file contexts. |
