...
| `Extension.MarkdownFlavor.AutoDetection` | Auto Detect infers flavor from vault, workspace, and syntax/context signals | `docs/bdd/features/ofmarkdown-language-mode.feature`, `src/vault/__tests__/document-membership.test.ts`, `src/lsp/handlers/__tests__/configuration.handler.test.ts`, `extension/src/markdown-flavor-evidence.test.ts`, planned `extension/src/markdown-flavor.test.ts`, planned inference classifier tests | 🔴 failing | Markdown flavor requirements, Phase 20 | Server-side auto resolution covers generic CommonMark, Obsidian markers, and project TOML evidence; fixture inventory now covers TOML-absent inference samples and boundary-negative README behavior. Syntax/context inference implementation and host proof remain pending. |
| `Extension.MarkdownStructuredProfiles.Configuration` | Extension configures Keep a Changelog, Common Changelog, and MADR as independent profile flags | `extension/src/markdown-flavor-evidence.test.ts`, planned `extension/src/markdown-flavor.test.ts`, `extension/docs/tests/markdown-flavor-unit-spec.md`, `extension/docs/tests/markdown-flavor-integration-spec.md` | 🔴 failing | Structured profile requirements | Fixture inventory now covers both changelog variants and MADR under every configured and TOML-absent inference smoke workspace. Configuration schema, host propagation, explicit overrides, and user-visible proof remain pending. |
...
| Planguage Tag | Requirement Gist | Test File(s) | Status | Phase | Notes |
|---|---|---|---|---|---|
| `FlavorLSP.StructuredProfiles.Flags` | Keep a Changelog, Common Changelog, and MADR layer over any base Markdown flavor without expanding the flavor list | `docs/design/markdown-structured-profile-flags.md`; `docs/test/markdown-flavor-unit-spec.md`; `docs/test/markdown-flavor-integration-spec.md`; `extension/src/markdown-flavor-evidence.test.ts`; planned structured-profile parser/LSP tests | 🔴 failing | Structured profile requirements | Spec and fixture inventory cover both changelog variants and MADR across configured and TOML-absent inference smoke workspaces. Contract, inference, diagnostics, symbols/folds, and server propagation implementation remain pending. |
