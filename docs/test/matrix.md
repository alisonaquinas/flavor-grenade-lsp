...
| `Workspace.VaultDetection.Fallback` | Directories with Flavor Grenade project config markers detected when `.obsidian/` absent | `src/vault/__tests__/vault-detector.test.ts` | ✅ passing | Phase 4 | Covers TOML, JSON, JSONC, YAML, YML, and `.editorconfig` markers. |
...
| Planguage Tag | Requirement Gist | Test File(s) | Status | Phase | Notes |
|---|---|---|---|---|---|
| `FlavorLSP.StructuredProfiles.Flags` | Keep a Changelog, Common Changelog, and MADR layer over any base Markdown flavor without expanding the flavor list | `docs/design/markdown-structured-profile-flags.md`; `docs/test/markdown-flavor-unit-spec.md`; `docs/test/markdown-flavor-integration-spec.md`; `extension/src/markdown-flavor-evidence.test.ts`; planned structured-profile parser/LSP tests | 🔴 failing | Structured profile requirements | Spec and fixture inventory cover both changelog variants and MADR across configured and project-config-absent inference smoke workspaces. Contract, inference, diagnostics, symbols/folds, and server propagation implementation remain pending. |
...
| `Security.Input.ProjectConfigSafety` | Project config files are confined, size-limited, schema-validated, and log-redacted | `src/lsp/handlers/__tests__/configuration.handler.test.ts`; `src/vault/__tests__/vault-detector.test.ts`; `src/test/integration/markdown-flavor.test.ts` | ✅ passing | Phase 20 | Project flavor evidence is read through a confined realpath path, capped, schema-filtered to `core.markdown`, supports TOML/JSON/JSONC/YAML/`.editorconfig`, and dangerous keys/invalid values are treated as absent configuration. |
