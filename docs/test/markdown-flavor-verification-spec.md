...
| MF-VF-008 | `src/test/ci-workflow.test.ts`, `docs/test/matrix.md`, and fixture inventory tests | Gate-removal checks protect syntax/context inference specs, inference fixture directories with no concrete `.fgattributes` flavor, ambiguity fallback fixtures, and boundary-negative fixtures such as the smoketest root README. |
