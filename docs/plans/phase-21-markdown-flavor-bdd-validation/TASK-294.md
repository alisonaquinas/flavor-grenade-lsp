...
- Add state for selector label, configured flavor, effective flavor,
  `.mdfattributes` target, and `.mdfignore` inactive state.
- Record simulated `.mdfattributes` writes and resource-specific propagation
...
- [ ] Manual non-Markdown language scenarios assert no `.mdfattributes` write and no
      server propagation.
...
- State shape: keep `languageId`, `configuredFlavor`, `effectiveFlavor`,
  `.mdfattributes` target writes, `.mdfignore` inactive state, and
...
> `languageId`, `configuredFlavor`, `effectiveFlavor`, `.mdfattributes` writes,
> `.mdfignore` inactive state, and server notifications separately.
