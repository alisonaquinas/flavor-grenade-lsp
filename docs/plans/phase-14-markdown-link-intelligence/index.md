---
title: Phase 14 - Markdown Link Intelligence Tickets
tags: [plans/phase-14, tickets/index]
aliases: ["Phase 14 Tickets"]
---

# Phase 14 - Markdown Link Intelligence Tickets

| Ticket | Title | Type | Status |
|---|---|---|---|
| [[FEAT-021]] | Markdown Link Intelligence | Feature | `in-progress` |
| [[TASK-156]] | Parse standard Markdown link syntax | Task | `green` |
| [[TASK-157]] | Classify Markdown link targets | Task | `green` |
| [[TASK-180]] | Complete Markdown link URL targets | Task | `green` |
| [[TASK-158]] | Index Markdown link references in RefGraph | Task | `green` |
| [[TASK-159]] | Resolve Markdown links through Oracle | Task | `green` |
| [[TASK-160]] | Diagnose Markdown heading anchors | Task | `green` |
| [[TASK-161]] | Navigate Markdown links and labels | Task | `green` |
| [[TASK-162]] | Rename Markdown heading anchors | Task | `green` |
| [[CHORE-044]] | Phase 14 Lint Sweep | Chore | `in-review` |
| [[BUG-002]] | Markdown index fields crash legacy OFMDoc fixtures | Bug | `in-progress` |
| [[BUG-003]] | Markdown path targets can escape above vault root | Bug | `in-progress` |
| [[BUG-004]] | Malformed Markdown anchor escapes can crash heading resolution | Bug | `in-progress` |
| [[BUG-005]] | Markdown completions lose folder context for nested documents | Bug | `in-progress` |
| [[CHORE-056]] | Phase 14 Code Quality Sweep | Chore | `in-progress` |
| [[CHORE-058]] | Document and split Phase 14 parser surfaces | Chore | `open` |
| [[CHORE-057]] | Phase 14 Security Sweep | Chore | `open` |
| [[CHORE-045]] | Phase 14 Test Matrix Sweep | Chore | `open` |
| [[CHORE-046]] | Phase 14 Documentation Trace Sweep | Chore | `open` |
