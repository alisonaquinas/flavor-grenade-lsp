...
|---|---|---|
| OFM wiki-links `[[Note]]` | Yes | Partial |
| Alias resolution `[[Note\|Display]]` | Yes | No |
| Transclusion embeds `![[Note]]` | Yes | No |
| Block references `![[Note#^blockid]]` | Yes | No |
| Block ID completion `^blockid` | Yes | No |
| Callout syntax `[!TYPE]` | Yes | No |
| Tag hierarchy `#parent/child` | Yes | No |
| Diagnostic codes (FG001–FG007) | Yes | No |
| `.flavor-grenade.toml` configuration | Yes | No |
| Standard CommonMark links | Yes | Yes |
| Heading references `[[Note#Heading]]` | Yes | Yes |
