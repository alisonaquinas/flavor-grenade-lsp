...
function parseFrames(buf: Buffer): { messages: unknown[]; remaining: Buffer } {
  ...
  while (true) {
    ...
    for (let i = 0; i <= current.length - 4; i++) {
      if (current[i]===0x0d && current[i+1]===0x0a && current[i+2]===0x0d && current[i+3]===0x0a) { sep=i; break; }
    }
    if (sep === -1) break;
...
export class FGWorld extends World {
  ...
  waitForNotification(method: string, timeoutMs = 10000): Promise<unknown> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.notifListeners.push({ method, resolve: (m) => { clearTimeout(t); resolve(m); } });
    });
  }
  ...
  waitForDiagnostics(uri: string, timeoutMs = 10000): Promise<unknown[]> {
    ...
    return new Promise((resolve, reject) => {
      const t = setTimeout(() => reject(new Error(`Timeout waiting for diagnostics: ${uri}`)), timeoutMs);
      const check = (msg: unknown): void => {
        const m = msg as Record<string, unknown>;
        const params = m['params'] as { uri: string; diagnostics: unknown[] };
        if (params.uri === uri) { clearTimeout(t); resolve(params.diagnostics); }
        else this.notifListeners.push({ method: 'textDocument/publishDiagnostics', resolve: check });
      };
  ...
  waitForExit(): Promise<number> {
    return new Promise(resolve => { this.proc!.on('exit', code => resolve(code ?? 1)); });
  }
  ...
  async startServer(rootUri?: string): Promise<void> {
    ...
    this.lastResponse = await this.request('initialize', { processId: null, rootUri: rootUri ?? null, capabilities: {} });
    ...
    if (rootUri) {
      ...
      try { await this.waitForNotification('flavorGrenade/status', 15000); } catch { /* may not come */ }
      try { await this.request('flavorGrenade/awaitIndexReady'); } catch { /* optional */ }
    }
  }
  ...
  async openDocument(relPath: string): Promise<void> {
    ...
    this.notify('textDocument/didOpen', { textDocument: { uri, languageId: 'markdown', version: 1, text } });
  }
