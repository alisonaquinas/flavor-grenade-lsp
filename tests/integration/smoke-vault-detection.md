...
**Setup:**
A temporary directory containing a single markdown file with a broken wiki-link. No `.obsidian/`, `.mdfignore`, or `.mdfattributes` anywhere in the path. This directly tests that the VaultDetector correctly falls back to `single-file` mode and that FG001 is suppressed.
...
And no ".mdfignore" or ".mdfattributes" exists at or above "/tmp/fg-smoke-004/"
...
2. Agent verifies no `.obsidian/`, `.mdfignore`, or `.mdfattributes` exists in `/tmp/fg-smoke-004` or its parents (up to `/tmp/`)
