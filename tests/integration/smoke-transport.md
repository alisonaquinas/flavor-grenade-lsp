...
**Setup:**
A temporary directory containing a single `.md` file but no `.obsidian/` directory and no `.mdfignore` or `.mdfattributes`. This tests the server's behaviour when vault detection fails.
...
And no ".mdfignore" or ".mdfattributes" file exists anywhere in "/tmp/fg-smoke-002"
