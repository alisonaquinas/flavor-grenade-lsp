...
Feature: Server detects a flavor-grenade-lsp workspace automatically
  ...
  Scenario: Author opens a non-Obsidian folder with an .mdfattributes marker and features activate
    Given a folder "Technical Docs/" containing:
      | path                                  | type |
      | Technical Docs/.mdfattributes           | file |
...
1. Agent creates two separate temporary directories.
   ...
   Directory B — `Technical Docs/`: contains `.mdfattributes` with `*.md flavor=auto` but no `.obsidian/`, plus `architecture/overview.md` and `architecture/decisions.md`.
...
**Pass:** In sub-scenario A, a folder with `.obsidian/` activates vault mode and makes cross-file completions available immediately. In sub-scenario B, a folder with only `.mdfattributes` does the same. No manual path configuration is required in either case.
