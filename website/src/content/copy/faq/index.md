...
No. Flavor Grenade reads Markdown files and project markers directly. An Obsidian vault can be detected through `.obsidian/`, and non-Obsidian Markdown projects can use `.mdfattributes` for flavor rules or `.mdfignore` for visibility rules.
...
Flavor Grenade can work with `original`, `commonmark`, `obsidian`, `gfm`, `glfm`, `pandoc`, `multimarkdown`, `mdx`, `kramdown`, `markdown-extra`, `r-markdown`, `reddit`, and `stack-overflow`. `.mdfattributes` can pin one of those flavors, and Auto Detect resolves to one effective flavor from Obsidian markers, strong syntax evidence, or CommonMark fallback when no concrete rule applies.
