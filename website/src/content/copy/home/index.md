...
`.mdfattributes` can pin flavor or structured-profile rules, `.mdfignore` hides files from processing, and Auto Detect runs whenever no concrete flavor rule applies. When there is no strong signal, generic Markdown falls back to CommonMark instead of being treated as Obsidian content.
