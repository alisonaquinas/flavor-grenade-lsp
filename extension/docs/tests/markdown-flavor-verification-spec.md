...
| EXT-MF-VF-010 | fixture inventory and host suite checks | CI/local gates protect configured smoketest fixtures, config-absent inference fixtures, ambiguous fallback fixtures, `.mdfignore` fixtures, and root README boundary-negative behavior from silent removal. |
| EXT-MF-VF-011 | structured profile fixture inventory checks | CI/local gates protect Keep a Changelog, Common Changelog, and MADR fixture inventory under every `.mdfattributes`-configured and config-absent inference smoke workspace. |
