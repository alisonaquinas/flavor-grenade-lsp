...
| EXT-MF-U-015 | Smoketest fixture inventory | `Extension.MarkdownFlavor.AutoDetection` | `extension/test-fixtures/workspaces/smoketest/` contains `.fgattributes` fixtures for every explicit flavor plus `inference/` fixtures with no `.fgignore` or `.fgattributes` for strong syntax/context inference and ambiguity fallback. |
