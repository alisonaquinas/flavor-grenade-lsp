...
|---|---|---|---|
| Unit | `extension/src/markdown-flavor.test.ts`; `extension/src/markdown-flavor-evidence.test.ts`; `extension/src/language-mode.test.ts`; updated contribution tests | 🟡 partial | Pure extension logic covers selector, flavor ids, configured auto-detection, persistence, propagation calls, document scope, and language preservation. Fixture inventory covers inference inputs; actual syntax/context inference remains planned. Contribution scoping remains E16. |
...
| Validation | BDD scenarios plus research-source trace review, inference smoke review, package-target evidence, stale expectation scan, and sanitized logs/artifacts | 🔴 needs step updates | Acceptance evidence proves required flavor ids, extension contract compatibility, syntax/context inference behavior, package-target proof, stale `ofmarkdown` retirement, and redaction of user paths, vault content, TOML content, environment variables, and API-like tokens. Server dialect semantics remain owned by root/server phases. |
