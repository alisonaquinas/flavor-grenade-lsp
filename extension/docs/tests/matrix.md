...
| Validation | BDD scenarios plus research-source trace review, package-target evidence, and stale expectation scan | 🔴 needs step updates | Acceptance evidence proves required flavor ids, extension contract compatibility, package-target proof, and stale `ofmarkdown` retirement. Server dialect semantics remain owned by root/server phases. |
