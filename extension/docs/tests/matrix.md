...
| `Extension.MarkdownFlavor.AutoDetection` | `extension/src/markdown-flavor.test.ts`; `extension/src/markdown-flavor-evidence.test.ts`; existing server membership tests; planned inference resolver tests | 🟡 partial | Resolver covers project evidence, Obsidian marker/membership evidence, invalid values, and CommonMark fallback. Fixture inventory now covers TOML-absent inference inputs and boundary-negative samples; inference implementation remains planned. |
| `Extension.MarkdownStructuredProfiles.Configuration` | `extension/src/markdown-flavor-evidence.test.ts`; planned `extension/src/markdown-flavor.test.ts`; `EXT-MF-U-017` through `EXT-MF-U-019`; `EXT-MF-I-013`; `EXT-MF-E-013` | 🔴 failing | Fixture inventory covers Keep a Changelog, Common Changelog, and MADR under every configured and TOML-absent inference smoke workspace. Configuration schema, structured-profile propagation, selector non-expansion, explicit overrides, and host proof remain pending. |
