...
| EXT-MF-I-010 | `extension/src/workspace-environment.test.ts` or startup tests | Restricted or virtual workspace with Markdown files and selector state. | Flavor selector state does not cause server spawn or propagation in unsupported environments. |
...
- Server-unavailable and restricted/virtual workspace paths keep language ids
  stable and avoid stale propagation.
