...
| EXT-MF-I-003 | `extension/src/activation-gate.test.ts` | User invokes flavor selector command. | Extension wakes enough to show selector and resolve selected-file or directory `.fgattributes` target. |
...
| EXT-MF-I-008 | `extension/src/markdown-flavor.test.ts` or `extension/src/commands.test.ts` | Change selector values across every required explicit flavor while a client stub records outbound messages. | Extension writes the expected `.fgattributes` scope and sends the documented refresh payload with resource-specific selected and effective flavor state. |
...
| EXT-MF-I-010 | `extension/src/workspace-environment.test.ts` or startup tests | Restricted, virtual, unsupported-scheme, or untrusted workspace with Markdown files and selector state. | Flavor selector state does not cause server spawn, `.fgattributes` writes, or propagation in unsupported/untrusted environments. |
| EXT-MF-I-011 | `extension/src/markdown-flavor-evidence.test.ts` and planned inference resolver tests | Config-absent smoketest inference fixtures are opened from bounded fixture roots. | Extension detects no applicable `.fgignore`/`.fgattributes`, passes bounded syntax evidence to Auto Detect, and expects strong fixtures to infer their target flavor while ambiguous fixtures remain CommonMark. |
| EXT-MF-I-012 | `extension/src/language-mode.test.ts` or startup tests | Open `extension/test-fixtures/workspaces/smoketest/README.md` as the workspace root while child fixtures and repository ancestor config files exist. | Root README remains generic Markdown/CommonMark and does not inherit attributed-flavor propagation from child `.fgignore`/`.fgattributes` files or ancestor markers. |
| EXT-MF-I-013 | `extension/src/markdown-flavor.test.ts` or host wiring tests | Open a Markdown file matched by `.fgignore`, then remove or negate the ignore pattern. | Extension shows inactive state while ignored, suppresses active flavor writes, then refreshes back to Auto Detect after re-inclusion. |
| EXT-MF-I-014 | `extension/src/markdown-flavor.test.ts` or planned structured-profile tests | Open Keep a Changelog, Common Changelog, and MADR fixtures from `.fgattributes`-configured and config-absent inference smoke workspaces under multiple base flavor selections. | Extension computes structured profile flags separately from effective base flavor, propagates both to the server, never exposes structured profile ids as Markdown flavor selector choices, and keeps sibling changelog variants from enabling both changelog flags at once. |
