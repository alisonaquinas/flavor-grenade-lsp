...
| MarkdownFlavorController | Owns visible selector state, writes scoped `.mdfattributes` rules, sends refresh inputs, and preserves VS Code's `markdown` language mode |
...
`.mdfattributes` writes, and refresh requests. BC4 owns the root
[Markdown flavor auto-detection algorithm](../../../docs/design/markdown-flavor-auto-detection.md)
and runs Auto Detect independently whenever no concrete `.mdfattributes`
`flavor` applies.
...
`.mdfattributes` in the active file's directory. Choosing Auto Detect removes or
