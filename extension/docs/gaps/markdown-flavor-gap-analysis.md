...
| `.mdfattributes` write path | Missing from selector implementation | High |
...
| `extension/package.json` | Does not expose current `.mdfattributes` selector write behavior. |
...
| GAP-E-004 | `Extension.MarkdownFlavor.AutoDetection`; `EXT-MF-U-004`, `EXT-MF-U-005` | Marker/server membership results map to language promotion, not effective flavor. | Auto detection must resolve `obsidian`, `.mdfattributes flavor=auto`, strong syntax inference, or `commonmark`. | High |
| GAP-E-005 | `Extension.MarkdownFlavor.OverridePersistence`; `EXT-MF-U-006` to `EXT-MF-U-008` | No selector write path exists. | Selector writes selected-file or directory `/*.md` rules to `.mdfattributes`; Auto clears/resets the same scope. | High |
...
| GAP-E-011 | `Extension.Tests.HostCoverage`; `EXT-MF-E-001` to `EXT-MF-E-006` | Host suite has `activation-language-mode.test.js`; no `markdown-flavor.test.js`. | Host tests must prove selector labels, scope prompt, `.mdfattributes` writes, server refresh, `.mdfignore` inactive state, and language preservation. | High |
...
| Validation | `extension/docs/tests/markdown-flavor-validation-spec.md` | No selector screenshot, `.mdfattributes` scope smoke record, host log, or research-backed display review exists. |
...
| 3 | Add `.mdfattributes` read/write support for required flavor ids, `flavor=auto`, and `structured_profiles`. |
| 4 | Add selector UI through a status item or equivalent command, quick-pick choices, second scope prompt, `Auto Detect (EffectiveFlavor)` display, and disabled/inactive behavior for non-Markdown or `.mdfignore` documents. |
| 5 | Implement override persistence to selected-file and directory `/*.md` `.mdfattributes` scopes, with same-scope reset for Auto. |
...
- `.mdfattributes` persists selected-file and directory-scope choices correctly;
...
- host tests prove visible selector behavior, `.mdfattributes` scope, and no custom
