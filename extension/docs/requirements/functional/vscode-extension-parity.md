...
3. Open a workspace containing `.mdfignore` or `.mdfattributes`.
...
2. Verify at least one test exists for activation in `.obsidian/`, activation by `.mdfignore` or `.mdfattributes`, generic Markdown isolation, required Markdown flavor selection, command registration, status transition, and missing server path failure.
...
**User Req:** User.Extension.StartOnlyForVaults
**Gist:** The extension manifest and activation controller must react to `.obsidian/`, `.mdfignore`, `.mdfattributes`, `markdown`, flavor selector commands, and explicit command activation signals.
...
1. Run extension-host fixtures for `.obsidian/`, `.mdfignore`, `.mdfattributes`, generic Markdown, flavor selector command activation, and explicit command activation.
...
5. Verify the selector writes or resets `.mdfattributes` for the chosen scope and the active document remains `languageId = markdown`.
...
4. Select each explicit flavor and verify `.mdfattributes` accepts the corresponding `flavor=<id>` value.
...
**Gist:** Markdown flavor overrides made from the extension UI must persist to `.mdfattributes` at selected-file or active-directory scope.
...
**Scale:** Percentage of selector write and clear operations that target the correct `.mdfattributes` file, pattern, and attribute.
**Meter:**

1. Open a Markdown file inside a workspace folder.
2. Select each explicit flavor from the selector.
3. Choose `Selected file` and verify `.mdfattributes` in the active file's directory contains a file-specific rule.
...
7. Verify `.mdfattributes` contains a directory-local Markdown pattern such as `/*.md flavor=<id>`.
8. Open a standalone Markdown file with no owning workspace folder and verify the same `.mdfattributes` rules are written beside that file.
9. Compute: (correct `.mdfattributes` operations / total write and clear operations) x 100.
**Fail:** A selector override is written to `flavorGrenade.markdownFlavor`, the wrong `.mdfattributes` file, a broader pattern than requested, or an unsupported flavor id.
...
**Gist:** The extension must surface `.mdfignore` matches as inactive Flavor Grenade documents and must not request active language behavior for ignored files.
...
1. Open Markdown files matched by root and nested `.mdfignore` patterns.
...
4. Verify the extension does not offer active flavor selector writes for ignored files unless the user edits `.mdfignore` first.
5. Remove or negate the ignore rule and verify the file returns to Auto Detect or `.mdfattributes` behavior after refresh.
...
**Gist:** In `auto` mode, the extension UI must display and propagate the effective Markdown flavor inferred from vault, `.mdfattributes`, and standalone-file context while allowing explicit user overrides to take precedence.
...
1. Open a Markdown document under a `.obsidian/` folder with no matching `.mdfattributes` flavor.
2. Verify the status-bar selector reports `Auto Detect` with effective flavor `obsidian`.
3. Open a Markdown document matched by `.mdfattributes` `flavor=auto`.
...
5. Open a directory tree with no `.mdfignore` or `.mdfattributes`.
6. Verify every Markdown file in that tree defaults to Auto Detect and generic Markdown resolves to `commonmark`.
7. Override the active context to each explicit required flavor and verify the `.mdfattributes` override takes precedence over auto detection until cleared.
...
2. Verify `.mdfattributes` `structured_profiles` accepts `auto`, `none`, or a unique compatible list using `keep-a-changelog`, `common-changelog`, and `madr`.
...
6. Verify explicit `.mdfattributes` structured-profile values override automatic inference and propagate to the server with the active document resource.
7. Verify selected-file and directory scope rules mirror `.mdfattributes` `flavor`.
