...
| Area | Requirement |
|---|---|
| Activation | Activate for `.obsidian/`, `.mdfignore`, and `.mdfattributes`; avoid expensive startup for generic Markdown |
| Markdown flavor | Keep `.md` files in `markdown`; expose Auto Detect and every supported researched Markdown flavor in a separate status-bar selector that reports the effective flavor, then prompt for selected-file or directory scope before writing `.mdfattributes` |
