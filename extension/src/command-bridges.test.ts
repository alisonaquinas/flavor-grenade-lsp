...
describe('native reference and link bridges', () => {
  ...
  it('opens valid link and embed targets with native document navigation', async () => {
    ...
    assert.equal(
      await handlers.followLink({ target: locationPayload('file:///vault/link.md') }),
      true,
    );
    assert.equal(
      await handlers.openEmbedTarget({ target: locationPayload('file:///vault/embed.png') }),
      true,
    );
...
describe('OFMarkdown graph command bridges', () => {
  it('bridges backlinks, outlinks, vault reveal, and diagnostic copy actions', async () => {
    ...
    assert.deepEqual(
      ...
      ['editor.action.showReferences', 'editor.action.showReferences', 'revealInExplorer'],
    );
